...
Description : IDX Stock Quantitative Analysis & Signal Generation
              Comprehensive Model & Portfolio Performance Evaluation Engine
              Version v2026.Q3.v16.4 (DINO Master Rule & Gemini 3.6 Flash Integrated)
...
def sanitize_evaluation_inputs(
    ...
    if preds.height > 0:
        ...
        if pred_col not in preds.columns:
            fallback = next((c for c in ["prediction", "score", "signal", "predicted_return", "prediction_probability"] if c in preds.columns), None)
            if fallback:
...
class GeminiPerformanceNarrativeEngine:
    ...
    def generate_narrative_report(self, evaluation_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Menghasilkan laporan diagnostik kualitatif berbasis AI Gemini dengan Fallback Model."""
        if not self.client:
            return {
        ...
        prompt = f"""
        Sebagai Portfolio Manager & Chief Risk Officer pasar saham BEI (IDX), buatkan evaluasi diagnostik ringkas (maksimal 2-3 paragraf) berdasarkan metrik kuantitatif berikut:
            ...
            try:
                ...
                if response and hasattr(response, "text") and response.text:
                    ...
                    return {
                        ...
                        "ai_model_used": target_model
                    }
            except Exception as e:
                logger.warning(f"⚠️ Error Gemini API pada Evaluation Engine ({target_model}): {e}")
...
class UnifiedEvaluationEngine:
    ...
    def __init__(
        ...
        logger.info("UnifiedEvaluationEngine v2026.Q3.v16.4 (Gemini AI Integrated) initialized successfully.")

    def run_full_evaluation(
        self,
        predictions_df: Union[pl.DataFrame, Any] = None,
        actuals_df: Optional[Union[pl.DataFrame, Any]] = None,
        benchmark_returns: Optional[np.ndarray] = None,
        pred_col: str = "predicted_return",
        actual_col: str = "realized_return",
        date_col: str = "signal_date",
        market_df: Optional[Union[pl.DataFrame, Any]] = None,
        signals_df: Optional[Union[pl.DataFrame, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if predictions_df is None and signals_df is not None:
            predictions_df = signals_df
        if actuals_df is None and market_df is not None:
            actuals_df = market_df
        ...
        with self._lock:
            ...
            payload = {
                "evaluation_timestamp": datetime.now(WIB_TZ).isoformat(),
    ...
    # -------------------------------------------------------------------------
    # ALIAS METHODS UNTUK COMPATIBILITY REFLEKSI orchestrator main.py
    # -------------------------------------------------------------------------
    def execute_pipeline_evaluation(self, **kwargs) -> Dict[str, Any]:
        return self.run_full_evaluation(**kwargs)

    def evaluate_pipeline(self, **kwargs) -> Dict[str, Any]:
        return self.run_full_evaluation(**kwargs)
