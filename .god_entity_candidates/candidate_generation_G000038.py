...
class CapabilityMetrics:
    """Tracks performance metrics and execution evidence for agent capabilities."""
    ...
    def record_execution(self, capability_name: str, success: bool, latency: float, metadata: Optional[Dict[str, Any]] = None) -> None:
        self.execution_counts[capability_name] = self.execution_counts.get(capability_name, 0) + 1
        if success:
            self.success_counts[capability_name] = self.success_counts.get(capability_name, 0) + 1
        
        if capability_name not in self.latencies:
            self.latencies[capability_name] = []
        self.latencies[capability_name].append(latency)
        ...
        logger.debug("Recorded execution for %s: success=%s, latency=%.4fs", capability_name, success, latency)

    def get_reliability(self, capability_name: str) -> float:
        total = self.execution_counts.get(capability_name, 0)
        if total == 0:
            return 0.0
        successes = self.success_counts.get(capability_name, 0)
        return successes / total

    def get_average_latency(self, capability_name: str) -> float:
        lats = self.latencies.get(capability_name, [])
        if not lats:
            return 0.0
    ...
    def generate_report(self) -> Dict[str, Any]:
        report = {}
        for cap in self.execution_counts:
            report[cap] = {
                "executions": self.execution_counts[cap],
                "reliability": self.get_reliability(cap),
                "avg_latency": self.get_average_latency(cap)
            }
        return report


class ReliableExecutionPipeline:
    """Wraps functions with evidence-driven retries, validation, and metrics tracking."""

    def __init__(self, metrics: Optional[CapabilityMetrics] = None) -> None:
        self.metrics = metrics or CapabilityMetrics()
        ...
        for attempt in range(retries + 1):
            ...
            try:
                ...
                if validator is not None and not validator(result):
                    raise ValueError(f"Result validation failed for capability: {capability_name}")
                success = True
                latency = time.time() - attempt_start
                self.metrics.record_execution(capability_name, True, latency, {"attempt": attempt + 1})
                break
            except Exception as e:
                last_exception = e
                latency = time.time() - attempt_start
                self.metrics.record_execution(capability_name, False, latency, {"attempt": attempt + 1, "error": str(e)})
                if attempt < retries:
                    time.sleep(backoff_factor * (2 ** attempt))
                else:
                    logger.error("Capability %s failed after %d retries. Last error: %s", capability_name, retries, e)
...
# GENERATION G000038 | parent lineage | objective: Continuously improve capabilities, reliability, and evidence-driven evolution.
