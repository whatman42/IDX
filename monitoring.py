...
Version          : 2026.Q3.v1.5.2 (IDX & Proxy-Aware Egress Health Check - Production Grade Patched)
...
class ModelMonitor:
    ...
    def evaluate_prediction_drift(
        ...
        # PERBAIKAN: Sanitasi nilai NaN/Inf sebelum pengujian statistik KS
    ...
    def _compute_classification_metrics(self, preds: np.ndarray, acts: np.ndarray) -> Dict[str, Any]:
        # PERBAIKAN: Konversi kontinu target return float (misal 0.02) menjadi biner 1/0
        y_pred = np.where(preds >= 0.5, 1, 0) if np.issubdtype(preds.dtype, np.floating) else preds.astype(np.int64)
        y_true = (acts > 0.0).astype(np.int64) if np.issubdtype(acts.dtype, np.floating) else acts.astype(np.int64)
...
class RuntimeMonitor:
    ...
    def compute_summary_statistics(self) -> Dict[str, Any]:
        ...
        try:
            # PERBAIKAN: Cast eksplisit boolean expression ke UInt32 sebelum sum
...
class DriftDashboard:
    ...
    def render_markdown(
        ...
        md.append(f"**Generated Time:** {summary['timestamp']}  ")
    ...
    def render_html(
        ...
        html.append(f"  <p><strong>Generated Timestamp:</strong> {summary['timestamp']}</p>")
        html.append("</div>")
