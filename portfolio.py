...
Version       : 2026.Q3.v23.0 (Single Unified Mode & Strict Zero-Position State)
...
class IDXExecutionEngine:
    ...
    def get_elapsed_years(self) -> float:
        try:
            start_dt = datetime.fromisoformat(self.start_timestamp_utc)
    ...
    def compute_almgren_chriss_impact_price(self, base_price: float, trade_nominal: float, volume_24h_idr: float, volatility_daily: float = 0.02, is_buy: bool = True) -> float:
        ...
        if volume_24h_idr <= 0 or trade_nominal <= 0:
            return round_to_idx_tick(base_price, round_mode="up" if is_buy else "down")
        
        part_ratio = min(trade_nominal / volume_24h_idr, MAX_ADTV_PARTICIPATION_PCT)
    ...
    def calculate_max_buyable_lots(self, ticker: str, target_nominal: float, execution_price: float, volume_24h_idr: float = float('inf')) -> Tuple[int, float, float]:
        ...
        if 0 < volume_24h_idr < self.min_adtv_idr and execution_price > 0:
            recalculated_idr = volume_24h_idr * execution_price
            if recalculated_idr >= self.min_adtv_idr:
                volume_24h_idr = recalculated_idr

        if volume_24h_idr < self.min_adtv_idr:
            if ticker not in self._logged_adtv_warnings:
                logger.warning(
                    f"⚠️ [ADTV REJECT] {ticker}: ADTV 24j (Rp {volume_24h_idr:,.0f}) "
        ...
        max_adtv_nominal = volume_24h_idr * MAX_ADTV_PARTICIPATION_PCT
    ...
    def execute_buy(self, ticker: str, target_nominal: float, current_price: float, volume_24h_idr: float = float('inf'), volatility_daily: float = 0.02) -> Optional[Dict[str, Any]]:
        with self._lock:
            ...
            execution_price = self.compute_almgren_chriss_impact_price(current_price, target_nominal, volume_24h_idr, volatility_daily, is_buy=True)
            lots, shares, total_cost = self.calculate_max_buyable_lots(ticker, target_nominal, execution_price, volume_24h_idr)
    ...
    def execute_sell(self, ticker: str, lots_to_sell: Optional[int], current_price: float, reason: str = "SIGNAL", volume_24h_idr: float = float('inf'), volatility_daily: float = 0.02) -> Optional[Dict[str, Any]]:
        with self._lock:
            ...
            execution_price = self.compute_almgren_chriss_impact_price(current_price, trade_nominal, volume_24h_idr, volatility_daily, is_buy=False)

            sell_shares = sell_lots * IDX_LOT_SIZE
            gross_amount = sell_shares * execution_price
            fee = gross_amount * self.sell_fee_pct
            net_proceeds = gross_amount - fee

            avg_buy_price = pos.get("avg_price", pos.get("buy_price", execution_price))
...
class CorrelationEngine:
    ...
    def compute_correlation(self, cov_df: pl.DataFrame):
        asset_names = cov_df["asset"].to_list()
        cov = cov_df.select([c for c in cov_df.columns if c != "asset"]).to_numpy()
...
class HierarchicalRiskParity:
    ...
    def allocate(self, cov_df: pl.DataFrame, corr_df: pl.DataFrame, dist_df: pl.DataFrame, signals_df: pl.DataFrame) -> pl.DataFrame:
...
class PortfolioOptimizer:
    ...
    def optimize_allocation(
        ...
        cov_clean = np.nan_to_num(cov_clean, nan=1e-6)
        n = cov_clean.shape[0]
        n_sig = hrp_df.height
...
class SectorConstraintsAuditor:
    ...
    def audit_sector_limits(self, metrics_df: pl.DataFrame, asset_metadata_df: pl.DataFrame) -> pl.DataFrame:
        sector_map = {}
        ...
        sectors = [_extract_symbol(r) for r in records]
        mapped_sectors = [sector_map.get(s, "FINANCIALS") for s in sectors]
...
# ==============================================================================
# UNIFIED PORTFOLIO ENGINE (FACADE CLASS - SINGLE UNIFIED MODE)
# ==============================================================================
class UnifiedPortfolioEngine:
    FACADE_VERSION: str = "2026.Q3.v23.0"

    def __init__(self, config: Optional[Dict[str, Any]] = None, state_file: Optional[str] = None) -> None:
        self._lock = threading.RLock()
    ...
    def process_trading_signals(
        ...
        with self._lock:
            ...
            for r in top_buy_records:
                ...
                vol_idr = raw_vol * raw_price if (0 < raw_vol < 1e7 and raw_price > 0) else raw_vol
            ...
            for r in records:
                ...
                if norm_sym in target_weights_map:
                    ...
                    if current_price > 0:
                        ...
                        if success:
                            executed_buys += 1
                            logger.info(f"🏆 [TOP-1 EXECUTED] Saham Peringkat 1 {norm_sym} berhasil dialokasikan.")
    ...
    def execute_pipeline(
        ...
        **kwargs
        ...
        with self._lock:
            if signals_df is None or signals_df.height == 0:
                logger.warning("⚠️ [PORTFOLIO PIPELINE] signals_df kosong. Mengembalikan DataFrame kosong.")
                return signals_df if isinstance(signals_df, pl.DataFrame) else pl.DataFrame()
    ...
    def run_portfolio_stress_test(self, returns_df: pl.DataFrame, asset_metadata_df: Optional[pl.DataFrame] = None) -> Dict[str, Any]:
        ...
        return StressTestEngine.run_stress_test(self.total_equity, w, cov, sectors=sectors)
