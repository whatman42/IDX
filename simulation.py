...
Directory     : Flat Directory (Root Level with main.py, signal_idx.py, risk.py)
Version       : 2026.Q3.v2.2.0 (Institutional Production Grade - Patched)
...
class FeeStructureModel:
    ...
    ENGINE_VERSION: str = "2026.Q3.v2.2.0"
...
# ==============================================================================
# PORTFOLIO TRACKER & ACCOUNTING ENGINE
# ==============================================================================
...
# ==============================================================================
# CORPORATE ACTION & MARKET STATE ENGINE
# ==============================================================================
...
# ==============================================================================
# REBALANCING & WALK-FORWARD ENGINE
# ==============================================================================
...
class DailyExecutionSimulator:
    ...
    def simulate(self, market_df: Union[pl.DataFrame, Any], signals_df: Union[pl.DataFrame, Any]) -> pl.DataFrame:
        ...
        for row in joint.iter_rows(named=True):
            ...
            # 1. Compute Slippage & Base Execution Price
            ep, slip, _ = self.slippage_engine.compute_execution_price(
                base_price=bp, order_size_idr=alloc, adv20_idr=adv, daily_volatility=vol, is_buy=True
            )

            # 2. Compute Fill Ratio & Partial Fill Executed Lots
            ...
            actual_executed_notional = actual_units * final_ep

            # PERBAIKAN CRITICAL: Biaya transaksi dihitung dari Notional Eksekusi Riil
            actual_fee = self.slippage_engine.fee_model.calculate_fee(actual_executed_notional, is_buy=True)
            ...
            if actual_units > 0:
                self.portfolio_tracker.add_buy_position(row["asset"], actual_units, final_ep, actual_fee)
...
class IDXSimulationEngine:
    ...
    FACADE_VERSION: str = "2026.Q3.v2.2.0"
