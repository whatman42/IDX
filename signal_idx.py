"""
================================================================================
MODULE: signal_idx.py (v2026.5.19 Optimized Sensitivity & AI Diagnostic Edition)
DESCRIPTION: Unified Signal Processing Engine for Indonesian Stock Market (IDX)
             with Google Gemini AI Signal Reasoning & Compliance Diagnostics
...
# Menurunkan batas ADTV harian agar saham berpotensi tidak langsung terbuang
...
def get_prob_expr(cols: List[str]) -> pl.Expr:
    ...
    if "prediction_probability" in cols:
        return pl.col("prediction_probability").fill_nan(0.53).fill_null(0.53)
    elif "probability" in cols:
        return pl.col("probability").fill_nan(0.53).fill_null(0.53)
    return pl.lit(0.53)
...
class GeminiSignalDiagnosticEngine:
    ...
    Menghasilkan diagnosis naratif kepatuhan regulasi BEI, risk-reward validity, dan keyakinan eksekusi.
    """
...
class SignalGenerator(_SignalBaseEngine):
    def __init__(self, engine_id: str, operational_config: Dict[str, Any]) -> None:
        super().__init__(engine_id, "v2026.5.19")
    ...
    def generate(self, upstream_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 1. Safe Auto-Imputation (Impute ke 0.53)
        adapter_exprs = []
        if "prediction_probability" not in cols and "probability" not in cols:
            adapter_exprs.append(pl.lit(0.53).cast(pl.Float64).alias("prediction_probability"))
            adapter_exprs.append(pl.lit(0.53).cast(pl.Float64).alias("probability"))
        ...
        tp_mult = float(active_cfg.get("tp_atr_multiplier", 1.8))
...
class ExitFilter(_SignalBaseEngine):
    ...
    def filter_exit(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        min_rr = float(self._config.get("min_risk_reward_ratio", 0.50))
...
class ProbabilityFilter(_SignalBaseEngine):
    ...
    def filter_probability(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        min_prob = float(self._config.get("min_prediction_probability", 0.50))
        ...
        prob_valid = (p_win >= min_prob) & (ev_idr >= -10.0)
...
class ConfidenceFilter(_SignalBaseEngine):
    ...
    def filter_confidence(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        cols = input_df.columns
        min_conf = float(self._config.get("min_prediction_confidence", 0.50))
...
class SignalExplainer(_SignalBaseEngine):
    ...
    def explain(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        audit_json = pl.struct([
            ...
            pl.lit("v2026.5.19").alias("engine_version")
        ]).struct.json_encode()
...
class UnifiedSignalEngine:
    FACADE_VERSION: Final[str] = "v2026.5.19"
    ...
    def __init__(
        ...
        default_cfg = {
            ...
            "entry_filter": {}, 
            "exit_filter": {
                "min_risk_reward_ratio": 0.50
            }, 
            "prob_filter": {
                "min_prediction_probability": 0.50
            }, 
            "conf_filter": {
        ...
        self.activate_all()
        logger.info(f"UnifiedSignalEngine [{self.engine_id_prefix}] facade initialized (v{self.FACADE_VERSION}) with Gemini AI Diagnostics | Mode ADTV: Rp {active_min_adtv:,.0f} IDR.")
