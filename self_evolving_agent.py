...
- SearXNG / Wikipedia / DuckDuckGo research
...
# =============================================================================
# RESEARCH ENGINE (SearXNG / Wikipedia / DuckDuckGo)
# =============================================================================

class ResearchEngine:
    def __init__(self, network_available: bool):
        self.network = network_available
        self.searxng_instances = [u.strip().rstrip("/") for u in os.environ.get("SEARXNG_INSTANCES", "").split(",") if u.strip()]
    ...
    def research(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        for instance in self.searxng_instances:
            try:
                ...
                self.last_error = f"SEARXNG_FAIL: {type(e).__name__}"
        if not results:
        ...
        results = self._validate_urls(results)
        if not results:
            self.last_error = self.last_error or "NO_RESULTS"
            self.functional = False
        else:
            self.functional = True
        log.event(Event.RESEARCH_COMPLETED, query=query, results=len(results), functional=self.functional)
        return results
    ...
    def _search_wikipedia(self, query, max_results):
        ...
        try:
            ...
            for item in data.get("query", {}).get("search", []):
                ...
                results.append({"title": item["title"], "url": page_url, "snippet": item.get("snippet", ""), "source": "wikipedia", "retrieved_at": utc_now()})
            return results
        except Exception:
            return []
        ...
        try:
            ...
            for match in pattern.finditer(html)[:max_results]:
                ...
                if raw_url and title:
                    results.append({"title": title, "url": raw_url, "snippet": snippet, "source": "duckduckgo_html", "retrieved_at": utc_now()})
            return results
        except Exception:
            return []
    ...
    def _validate_urls(self, results):
        return [r for r in results if r.get("url", "").startswith(("http://", "https://"))]
...
class VerificationEngine:
    ...
    def verify(self, candidate_path: Path) -> Dict[str, Any]:
        ...
        if shutil.which("pytest"):
            ...
            results["checks"]["pytest"] = {"ok": code == 0, "output": out[-500:]}
        else:
...
class FailureIntelligence:
    ...
    def has_failed_before(self, category: str, context_key: str) -> bool:
        ...
        for f in failures:
            if context_key in f.get("context", ""):
                return True
        return False
...
class EvolutionEngine:
    ...
    def _generate_enhancement(self, objective, research_context):
...
"""
    ...
    def _apply_llm_changes(self, objective, research_context):
        if not self.model_provider or not self.model_provider.available():
            return []
        prompt = f"""
You are autonomous developer. Objective: {objective}
You may create new Python modules or modify existing ones.
Return JSON with optional arrays "create" and "modify".
Each create: {{"path": "relative/path.py", "content": "python code"}}
Each modify: {{"path": "relative/path.py", "search": "substring", "replace": "new content"}}
...
"""
        ...
        for item in data.get("create", []):
            path = (self.base_dir / item["path"]).resolve()
            if not self._is_safe_path(path):
                continue
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(item["content"], encoding="utf-8")
            changes.append(f"created:{item['path']}")
        for item in data.get("modify", []):
            path = (self.base_dir / item["path"]).resolve()
            if not self._is_safe_path(path) or not path.exists():
                continue
            text = path.read_text(encoding="utf-8")
            new_text = text.replace(item["search"], item["replace"])
            if new_text != text:
                path.write_text(new_text, encoding="utf-8")
                changes.append(f"modified:{item['path']}")
        return changes
    ...
    def promote(self, gen_id, candidate_path, test_results):
    ...
    def _deactivate_previous_generations(self, except_id):
...
class GitManager:
    ...
    def commit_if_enabled(self, message):
...
class RecoveryManager:
    def __init__(self, memory, evolution, core_path):
        self.memory = memory
    ...
    def run_loop(self, max_iterations=None):
        ...
        while True:
            ...
            if i % 3 == 0:
                ...
                history.append({"iteration": i, "validation": "active_core_valid"})
            i += 1
