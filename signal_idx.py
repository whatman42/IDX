...
class SignalGenerator(_SignalBaseEngine):
    ...
    def generate(self, upstream_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 1. Safe Auto-Imputation
        adapter_exprs = []
        if "prediction_probability" not in cols and "probability" not in cols:
            adapter_exprs.append(pl.lit(0.51).cast(pl.Float64).alias("prediction_probability"))
            adapter_exprs.append(pl.lit(0.51).cast(pl.Float64).alias("probability"))
