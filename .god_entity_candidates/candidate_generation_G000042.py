...
class ExecutionResult:
    """Encapsulates the result of a monitored execution for evidence-driven evolution."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "value": str(self.value) if self.success else None,
...
class ReliableExecutor:
    """A resilient execution wrapper that gathers telemetry and implements

    adaptive reliability patterns (retries, circuit breaking, fallback).
    """
    ...
    def _trip_circuit(self):
        self._circuit_open_until = time.time() + self.recovery_time_s
        logger.warning(
            f"Circuit breaker tripped. Pausing execution for {self.recovery_time_s}s."
        )
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        if self._is_circuit_open():
            err = RuntimeError("Circuit breaker is OPEN. Execution halted.")
            if fallback:
                try:
                    val = fallback(*args, **kwargs)
                    return ExecutionResult(True, value=val, error=None, metadata={"fallback": True})
                except Exception as fb_err:
                    return ExecutionResult(False, error=fb_err, metadata={"fallback_failed": True})
            return ExecutionResult(False, error=err, metadata={"circuit_open": True})
        ...
        while attempt <= self.max_retries:
            try:
                ...
                self._telemetry_log.append(result.to_dict())
                return result
        ...
        if fallback:
            try:
                ...
                result = ExecutionResult(
                    ...
                    error=last_error,
                    execution_time_ms=duration,
                    metadata={"fallback": True, "attempts": attempt},
                )
                self._telemetry_log.append(result.to_dict())
                return result
            except Exception as fb_err:
    ...
    def get_reliability_metrics(self) -> Dict[str, Any]:
        ...
        total = len(self._telemetry_log)
        successes = sum(1 for entry in self._telemetry_log if entry["success"])
        avg_time = sum(entry["execution_time_ms"] for entry in self._telemetry_log) / total
...
# GENERATION G000042 | parent lineage | objective: Continuously improve capabilities, reliability, and evidence-driven evolution.
