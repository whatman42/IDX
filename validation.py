"""
=============================================================================
IDX Quantitative Analysis System - Institutional Validation Engine
FileName      : validation.py
Directory     : Flat Directory (Root Level selevel dengan main.py)
Version       : 2026.Q3.v3.2.0 (Institutional Production Grade - Patched)
Compliance    : Indonesian Stock Exchange (IDX) Trading Rules & Econometrics
=============================================================================
"""
...
if not logger.handlers:
    ...
    handler.setFormatter(logging.Formatter('[%(asctime)s][%(levelname)s][IDX.VALIDATION] %(message)s'))
...
# ==============================================================================
IDX_FEE_ROUNDTRIP_PCT: float = 0.30 / 100.0     # 0.30% Total roundtrip trading fee
...
class ReproducibilityAudit:
    ...
    config_checksum: str = ""
    metadata_hash: str = ""
    timestamp_wib: str = field(default_factory=_get_wib_timestamp_str)
...
class BaseValidationEngine(ABC):
    ...
    def _validate_temporal_integrity(self, df: pl.DataFrame) -> None:
        ...
        is_duplicated = bool(df.select([self.asset_column, self.time_column]).is_duplicated().any())
        if is_duplicated:
        ...
        if not bool(check_monotonic["is_monotonic"].all()):
...
class TimeSeriesBootstrapEngine:
    ENGINE_VERSION: str = "2026.Q3.v3.2.0"
...
# ==============================================================================
# SUB-ENGINE 2: EVENT-BASED PURGED COMBINATORIAL CV
# ==============================================================================
...
# ==============================================================================
# SUB-ENGINE 3: FLEXIBLE WALK FORWARD VALIDATOR
# ==============================================================================
...
class MonteCarloEngine:
    ...
    def _fit_gaussian_hmm_parameters(self, returns: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        raw_bounds = np.percentile(returns, quantiles)
        
        # PERBAIKAN: Penanganan Robust Quantile Bounds apabila terjadi duplikasi
        bounds = np.unique(raw_bounds)
        if len(bounds) < n_states + 1:
            bounds = np.linspace(np.min(returns), np.max(returns) + 1e-8, n_states + 1)
        ...
        for k in range(n_states):
            ...
            means[k] = np.mean(subset) if len(subset) > 0 else 0.0
            stds[k] = max(1e-4, np.std(subset)) if len(subset) > 0 else 0.015
            states[mask] = k
...
class StatisticalValidationEngine:
    ...
    def execute_true_adf_test(self, series: np.ndarray, max_lags: int = 4) -> Dict[str, Any]:
        ...
        # PERBAIKAN CRITICAL: Penggunaan Pseudo-Inverse untuk mencegah matriks singular pada ADF
        sigma_sq = np.sum((Y - X @ beta_hat)**2) / max(1, (n - X.shape[1]))
        var_beta = sigma_sq * np.linalg.pinv(X.T @ X)
        
        t_stat = beta_hat[1] / np.sqrt(max(1e-12, var_beta[1, 1]))
        crit_5pct = -2.86
    ...
    def execute_kpss_test(self, series: np.ndarray) -> Dict[str, Any]:
        ...
        kpss_stat = np.sum(S**2) / (n**2 * sigma_sq) if sigma_sq > 1e-12 else 0.0
        crit_5pct = 0.463
    ...
    def execute_white_heteroskedasticity_test(self, residuals: np.ndarray, regressors: np.ndarray) -> Dict[str, Any]:
        ...
        # PERBAIKAN CRITICAL: Pembersihan Kolom Konstan & Deduplikasi Vektor Polynomial
        non_const_cols = [i for i in range(regressors.shape[1]) if np.std(regressors[:, i]) > 1e-8]
        if not non_const_cols:
            return {"white_lm_statistic": 0.0, "p_value": 1.0, "is_heteroskedastic": False}

        X_clean = regressors[:, non_const_cols]
        X_white_blocks = [np.ones((n, 1)), X_clean]
        for i in range(X_clean.shape[1]):
            X_white_blocks.append(X_clean[:, i:i+1] ** 2)

        X_mat = np.column_stack(X_white_blocks)

        _, res, _, _ = np.linalg.lstsq(X_mat, e_sq, rcond=None)
        ss_res = np.sum(res) if len(res) > 0 else np.sum((e_sq - X_mat @ np.linalg.pinv(X_mat) @ e_sq)**2)
        r_sq = 1.0 - ss_res / (np.sum((e_sq - np.mean(e_sq))**2) + 1e-12)
        lm_stat = n * max(0.0, r_sq)
        p_val = 1.0 - stats.chi2.cdf(lm_stat, df=max(1, X_mat.shape[1] - 1))
    ...
    def execute_arch_lm_test(self, series: np.ndarray, lags: int = 5) -> Dict[str, Any]:
        ...
        ss_res = np.sum(res) if len(res) > 0 else np.sum((Y - X @ np.linalg.pinv(X) @ Y)**2)
        r_sq = 1.0 - ss_res / (np.sum((Y - np.mean(Y))**2) + 1e-12)
        lm_stat = n * max(0.0, r_sq)
...
# ==============================================================================
# UNIFIED VALIDATION ENGINE (FACADE CLASS WITH GEMINI AI - v2026.Q3.v3.2.0)
# ==============================================================================
class UnifiedValidationEngine:
    ...
    def validate_full_pipeline(
        ...
        """
        audit_trail = ReproducibilityAudit(seed=self.seed, config_checksum="UNIFIED-FACADE-VAL-v3.2")
