...
Version       : 2026.Q3.v16.32 (Institutional Production-Grade & Gemini AI Integrated)
...
WIB_TZ: timezone = timezone(timedelta(hours=7), name="WIB")
...
def _extract_float(data: Dict[str, Any], keys: List[str], default: float = 0.0) -> float:
    """Ekstraksi nilai float opsional secara defensif dari dictionary."""
    for k in keys:
        val = data.get(k)
        if val is not None:
            try:
...
def escape_markdown_v2_text(text: Any) -> str:
    ...
    PENTING: Karakter backslash '\\' harus di-escape PERTAMA KALI.
    """
    if text is None:
        return ""
    s = str(text)
    # Escape backslash terlebih dahulu agar tidak merusak escaping selanjutnya
    s = s.replace("\\", "\\\\")
    special_chars = r"_*[]()~`>#+-=|{}.!"
...
class PortfolioStateCache:
    ...
    def validate_schema(self, data: Dict[str, Any], default_capital: float) -> PortfolioStateDict:
        ...
        if len(cleaned_positions) == 0:
            ...
            if top_pick == "-" and cleaned_positions:
...
class SignalSummaryGenerator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, mode: str = "dry_run") -> None:
        ...
        self.framework_version: str = str(self.config.get("FRAMEWORK_VERSION", "2026.Q3.v16.32"))
    ...
    def _validate_and_clean_data(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # PERBAIKAN: Menggunakan ekspresi Polars murni alih-alih map_elements UDF
        prob_expr = (
            pl.when(pl.col("probability").is_null() | pl.col("probability").is_nan())
            .then(0.5)
            .when(pl.col("probability") > 1.0)
            .then(pl.col("probability") / 100.0)
        ...
        conf_expr = (
            pl.when(pl.col("confidence").is_null() | pl.col("confidence").is_nan())
            .then(0.5)
            .when(pl.col("confidence") > 1.0)
            .then(pl.col("confidence") / 100.0)
            .otherwise(pl.col("confidence"))
            .clip(0.0, 1.0)
            .alias("confidence")
        )

        df = df.with_columns([prob_expr, conf_expr])
...
class TelegramReporter:
        ...
        for line in lines:
            ...
            if line_len > max_chars:
                ...
                for i in range(0, len(line), max_chars):
                    sub_line = line[i:i + max_chars]
                    # Mencegah pemotongan tepat di tengah karakter escape backslash
                    if sub_line.endswith("\\") and not sub_line.endswith("\\\\"):
                        sub_line = sub_line[:-1]
    ...
    def _adapt_orders_for_generator(self, orders: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for idx, item in enumerate(orders):
            ...
            # PERBAIKAN: Menggunakan _extract_float untuk menghindari float(None) TypeError
            price = _extract_float(item, ["entry_price", "price", "close", "current_price"], default=0.0)
            tp = _extract_float(item, ["target_price", "take_profit", "tp_price", "optimized_take_profit"], default=price * 1.05)
            sl = _extract_float(item, ["stop_loss", "sl_price", "optimized_stop_loss"], default=price * 0.95)
            ...
            rank = _extract_float(item, ["signal_rank_score", "ranking_score"], default=prob * conf)
