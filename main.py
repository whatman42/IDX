...
if __name__ == "__main__":
    ...
    if "--bootstrap-universe" in sys.argv:
        ...
        try:
            ...
            # Pemicu pencatatan / pemuatan universe
            if hasattr(data_engine, "get_universe"):
                data_engine.get_universe(force_refresh=True)
            elif hasattr(data_engine, "load_and_prepare_market_data"):
                # Fallback: panggil loader singkat untuk memastikan universe.json ter-write ke disk
                data_engine.load_and_prepare_market_data(symbols=DEFAULT_IDX_TICKERS[:10], use_cache=False)
            
            if os.path.exists("universe.json"):
                sys.stdout.write("✔ [BOOTSTRAP_SUCCESS] universe.json berhasil dibuat.\n")
                sys.exit(0)
            else:
