...
Version     : 2026.Q3.v17.3 (Gemini AI Meta-Diagnostic Integrated)
...
# ==============================================================================
# GEMINI AI META-DIAGNOSTIC ENGINE
# ==============================================================================
class GeminiAutonomousDiagnosticEngine:
    """
    Sub-Engine Google Gemini AI sebagai Meta-Diagnostic Layer yang mengevaluasi
    performa sistem perdagangan dan memberikan rekomendasi kualitatif/adaptif.
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if HAS_GEMINI_SDK and self.api_key:
            try:
                ...
                json_logger.log_event("INFO", "GEMINI_AUTONOMOUS_INIT", {"status": "Connected"})
            except Exception as err:
                json_logger.log_event("WARNING", "GEMINI_AUTONOMOUS_INIT_FAILED", {"error": str(err)})

    def run_meta_diagnostic(self, metrics: Dict[str, Any], risk_config: Dict[str, Any], drawdown_pct: float) -> Dict[str, Any]:
        """Melakukan analisis diagnostik mendalam menggunakan Google Gemini AI."""
        if not self.client:
            return {
                "ai_diagnostic": "Gemini AI Client tidak aktif.",
                "ai_recommendation": "Gunakan ambang batas risiko standar.",
                "suggested_risk_scale": 1.0
            }

        prompt = f"""
        Sebagai Chief Risk Officer & AI Trading System Architect BEI (IDX), lakukan analisis diagnostik terhadap evaluasi siklus perdagangan otonom berikut:

        === METRIK EVALUASI SIKLUS ===
        - Total Sinyal Dievaluasi: {metrics.get('total_evaluated_signals', 0)}
        - Rolling MAE (Error): {metrics.get('rolling_mae', 0.0):.4f}
        ...
        === KONFIGURASI RISIKO ADAPTIF SAAT INI ===
        - Min Confidence Required: {risk_config.get('min_required_confidence', 0.50):.2f}
        - Max Position Allocation: {risk_config.get('max_position_pct_idr', 0.25)*100:.1f}%

        Tugas:
        1. Berikan kesimpulan diagnostik singkat (maksimal 2 kalimat) mengenai kesehatan sistem & data drift.
        2. Berikan rekomendasi tindakan operasional (Lanjut/Pengetatan Risiko/Retraining).
        3. Berikan nilai skala alokasi risiko yang disarankan (antara 0.5 hingga 1.2).
        ...
        try:
            ...
            return {
                "ai_diagnostic": parsed.get("diagnostic", "Analisis berhasil."),
                "ai_recommendation": parsed.get("recommendation", "Lanjutkan operasional normal."),
                "suggested_risk_scale": float(parsed.get("risk_scale", 1.0))
            }
        except Exception as err:
            json_logger.log_event("WARNING", "GEMINI_DIAGNOSTIC_FAILED", {"error": str(err)})
            return {
                "ai_diagnostic": "Gagal memperoleh respon AI Gemini.",
                "ai_recommendation": "Pertahankan guardrail bawaan.",
                "suggested_risk_scale": 1.0
            }
...
class IDXSelfLearningEngine:
    """
    Autonomous Self-Learning Engine v2026.Q3.v17.3
    Mendukung evaluasi Polars Native, dekomposisi Error, Brier/ECE Calibration,
    Population Stability Index (PSI), Internal Equity Curve, Dynamic TP/SL Mode,
    dan integrasi Google Gemini Meta-Diagnostic Layer.
    """
    ...
    def __init__(
        ...
        tpsl_conflict_mode: str = "RANDOMIZED",
    ...
    def update_adaptive_risk_thresholds(self, metrics: Dict[str, Any], ai_risk_scale: float = 1.0) -> Dict[str, float]:
        """Skalasi Risiko Adaptif Berdasarkan Multi-Faktor Kuantitatif & Gemini AI Risk Scale."""
        with self._lock:
            ...
            # Integrasi Skala Risiko rekomendasi AI Gemini
            final_position_scale = position_scale * ai_risk_scale

            adjusted_max_position = max(0.05, min(0.30, (base_max_position_size * final_position_scale) / (mae_factor * calibration_penalty)))
    ...
    def run_autonomous_cycle(
        ...
        with self._lock:
            ...
            # Jalankan Meta-Diagnostic Layer via Google Gemini AI
            ai_diag = self.gemini_engine.run_meta_diagnostic(metrics, {}, effective_drawdown)

            risk_config = self.update_adaptive_risk_thresholds(metrics, ai_risk_scale=ai_diag.get("suggested_risk_scale", 1.0))
            ...
            json_logger.log_event("INFO", "AUTONOMOUS_CYCLE_COMPLETE", {
                ...
                "ai_diagnostic": ai_diag.get("ai_diagnostic")
            })
...
if __name__ == "__main__":
    json_logger.log_event("INFO", "MODULE_SELF_TEST_START", {"version": "v2026.Q3.v17.3"})
