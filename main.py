...
VERSION     : 2026.Q3.v2.5.2-PRODUCTION-FINAL (Aligned with Pipeline, Signal & Reset)
...
# =============================================================================
# HELPER: SYSTEM PREPARATION & PREDICTION LOG INITIALIZATION
# =============================================================================
def ensure_prediction_log_exists(log_file: str = "prediksi_idx_log.csv") -> None:
    """Memastikan file log prediksi selalu ada untuk mencegah fallback mode WORST_CASE."""
...
def _determine_execution_mode() -> str:
    """
    Menentukan mode eksekusi:
    - 'reset-dryrun' : Reset saldo simulasi & bersihkan cache usang.
    - 'self-learning': Retraining & optimasi model.
...
# =============================================================================
# 4. DRY-RUN BALANCE & PORTFOLIO STATE RESET HANDLER (FRESH RESET ENHANCED)
# =============================================================================
def reset_dryrun_balance(logger: logging.Logger) -> None:
    ...
    # 1. Purge Cache Data Pasar
    logger.info("🧹 [RESET] Membersihkan cache data pasar usang...")
    ...
    # 2. Re-initialize prediction log
    ensure_prediction_log_exists("prediksi_idx_log.csv")

    # 3. Re-initialize Portfolio State
    portfolio_state = {
...
class ProductionOrchestrator:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # Step 5: Multi-Horizon Prediction & Log Logging
    ...
    def _step_5_prediction(self) -> None:
        ...
        # =====================================================================
        # 🟢 APPEND OUTPUT PREDIKSI KE LOG PREDIKSI (prediksi_idx_log.csv)
        # =====================================================================
        try:
            if isinstance(predictions_df, pl.DataFrame) and predictions_df.height > 0:
                log_file = Path("prediksi_idx_log.csv")
                
                # Pastikan file ada dengan header standar jika belum tersedia
                ensure_prediction_log_exists(str(log_file))

                # Ambil snapshot baris terbaru untuk tiap ticker/asset
                ...
                # Ambil record data paling akhir dari DataFrame
                ...
                # Identifikasi nama kolom probabilistik dan konfidensi
                ...
                if rows_to_append:
                    ...
                    self.logger.info(f"✔ [LOG_PREDICTION_SUCCESS] Berhasil mencatat {len(rows_to_append)} riwayat prediksi ke '{log_file.name}'.")

        except Exception as log_err:
            self.logger.warning(f"⚠️ [LOG_PREDICTION_WARNING] Gagal memperbarui file log prediksi: {log_err}")
    ...
    def _step_12_self_learning(self) -> None:
        """Step 12: Continuous Model Optimization Engine (Filtered Numeric Features for Drift Analysis)."""
        try:
            try:
            ...
            if hasattr(sl_facade, "run_full_feedback_cycle"):
                ...
                if isinstance(features_df, pl.DataFrame):
                    ...
                    feature_cols = [
                        col for col, dtype in zip(features_df.columns, features_df.dtypes)
                        if dtype in numeric_dtypes and col not in ["date", "timestamp", "asset", "ticker", "symbol"]
                    ]
    ...
    def _step_15_monitoring(self) -> None:
        ...
        try:
            max_staleness = float(os.getenv("IDX_MAX_STALENESS_SEC", "86400.0"))
