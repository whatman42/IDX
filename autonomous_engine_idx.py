...
Version     : 2026.Q3.v17.1 (Institutional Hedge-Fund Grade)
...
IDX_MIN_24H_VOLUME_IDR: float = 1_000_000_000.0 # Batas Minimal Nilai Transaksi Harian (Rp 1 Miliar)
IDX_MAX_STALENESS_SEC: float = 86400.0       # Toleransi Keusangan Data Candle (24 Jam)
        ...
        with self._lock:
            try:
                ...
                """, (
                    ...
                    float(metrics.get("peak_equity_idr", 100_000_000.0)),
                    float(drawdown_pct),
...
class IDXSelfLearningEngine:
    """
    Autonomous Self-Learning Engine v2026.Q3.v17.1 (Hedge-Fund Grade)
    ...
    def __init__(
        ...
        max_allowed_mae_pct: float = 0.08,
        max_drawdown_kill_pct: float = 0.15,
        tpsl_conflict_mode: str = "WORST_CASE",  # Options: WORST_CASE, BEST_CASE, RANDOMIZED
        initial_equity_idr: float = 100_000_000.0,
    ):
    ...
    def load_and_normalize_logs(self) -> pl.DataFrame:
        ...
        with self._lock:
            try:
                # Direct read attempt to eliminate file-existence race conditions
                ...
                if "probability" in pldf.columns:
                    ...
                    pldf = pldf.with_columns(pl.lit(0.65).alias("probability"))
        ...
        with self._lock:
            ...
            for row in logs_pldf.iter_rows(named=True):
                ...
                for i, (hp, lp) in enumerate(zip(highs, lows), start=1):
                    ...
                    # Conflict Resolution for Intraday TP/SL Touch
                ...
                # Future Close Return (Baseline Market Price Return)
                future_close_return = (float(closes[-1]) - entry_p) / entry_p

                # Realized Net Return (BEI Asymmetric Fee Friction Model: 0.15% Buy + 0.15% Sell = 0.3% Roundtrip)
    ...
    def calculate_rolling_metrics_suite(self, evaluated_pldf: pl.DataFrame) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Mathematical Infinity for Profit Factor
            ...
            # Calmar Ratio (Annualized CAGR / MTM Drawdown)
            cagr = mean_ret * 252
            ...
            probs = recent_pldf["probability"].to_numpy() if "probability" in recent_pldf.columns else np.full(total_signals, 0.65)
    ...
    def update_adaptive_risk_thresholds(self, metrics: Dict[str, Any]) -> Dict[str, float]:
        ...
        with self._lock:
            ...
            base_max_position_size = 0.20  # 20% IDR per asset

            mae_ratio = mae / (self.max_allowed_mae_pct + EPSILON)
            mae_factor = min(2.0, max(0.5, mae_ratio))

            calibration_penalty = 1.15 if ece > 0.15 else 1.0
            drift_penalty = 1.20 if psi >= 0.20 else 1.0

            adjusted_confidence = min(0.88, max(0.50, base_confidence * mae_factor * calibration_penalty * drift_penalty))
    ...
    def check_composite_retraining_trigger(self, metrics: Dict[str, Any]) -> bool:
        ...
        with self._lock:
            ...
            retrain_required = sum(triggers) >= 2
    ...
    def check_emergency_kill_switch(self, metrics: Dict[str, Any], current_drawdown_pct: float) -> Tuple[bool, str]:
        ...
        with self._lock:
            ...
            if total_signals >= 15 and mae > (self.max_allowed_mae_pct * 1.5):
                reason = f"KILL-SWITCH: MAE ({mae:.4f}) melampaui batas kritis ({self.max_allowed_mae_pct*1.5:.4f})."
                json_logger.log_event("CRITICAL", "KILL_SWITCH_MAE", {"reason": reason})
                return True, reason

            if total_signals >= 25 and win_rate < 0.20:
                reason = f"KILL-SWITCH: Win Rate ({win_rate*100:.1f}%) di bawah batas minimal (20%)."
        ...
        with self._lock:
            ...
            if logs_pldf.is_empty():
                return {
                    ...
                    "risk_config": {"min_required_confidence": 0.60, "max_position_pct_idr": 0.20},
                }

            # 2. Evaluate Outcomes via Polars
            ...
            # 4. Update Internal Equity & Compute Drawdown
...
if __name__ == "__main__":
    json_logger.log_event("INFO", "MODULE_SELF_TEST_START", {"version": "v2026.Q3.v17.1"})
