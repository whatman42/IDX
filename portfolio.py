        ...
        with self._lock:
            ...
            for r in records:
                ...
                if norm_sym in target_weights_map:
                    ...
                    if current_price > 0:
                        # Duplicate protection: skip if already ACTIVE (no pyramiding by default)
                        existing = self.positions.get(norm_sym) or self.positions.get(normalize_idx_symbol(norm_sym))
                        if existing and str(existing.get("status", "ACTIVE")).upper() in ("ACTIVE", "OPEN"):
                            horizon_existing = str(existing.get("horizon", "SHORT"))
                            horizon_new = str(r.get("horizon") or r.get("expected_holding_days") or "SHORT")
                            if horizon_existing == str(horizon_new) or True:
                                # Default: one ACTIVE position per symbol (architecture does not pyramid)
                                logger.info(f"🛡️ [DUP_SKIP] {norm_sym} already ACTIVE — ignore duplicate BUY")
                                r["optimized_weight"] = 0.0
                                r["capital_allocation"] = 0.0
                        ...
                        if success:
                            executed_buys += 1
                            # Attach signal geometry to simulated position (simulation-only metadata)
                            pos = self.positions.get(norm_sym) or self.positions.get(normalize_idx_symbol(norm_sym))
                            if pos is not None:
                                ...
                                conf = _extract_float(r, ["confidence", "prediction_probability", "probability"], default=0.0)
                                rrr = _extract_float(r, ["risk_reward_ratio", "optimized_risk_reward", "calculated_risk_reward"], default=0.0)
                                pos["status"] = "ACTIVE"
                                pos["side"] = "BUY"
                                pos["tp_price"] = tp
                                pos["take_profit"] = tp
                                ...
                                pos["reason"] = str(r.get("reason") or r.get("signal_explanation") or r.get("signal_explanation_text") or r.get("final_validator_reason") or "SIMULATED_BUY")
                                pos["horizon"] = str(r.get("horizon") or r.get("expected_holding_days") or "SHORT")
                            ...
                            logger.info(f"🏆 [SIMULATED BUY ACTIVE] {norm_sym} | positions={len(self.positions)}")
