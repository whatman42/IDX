...
Version   : v2026.Q3.v25.5.2 (Institutional Production-Ready Edition with Gemini AI)
=============================================================================
"""
...
# -----------------------------------------------------------------------------
IDX_BUY_BROKER_FEE: Final[float] = 0.0015            # 0.15% buy broker fee
IDX_SELL_BROKER_FEE: Final[float] = 0.0015           # 0.15% sell broker fee
IDX_SELL_PPH: Final[float] = 0.0010                  # 0.10% PPh final on sell
IDX_FEE_ROUNDTRIP_PCT: Final[float] = (
    IDX_BUY_BROKER_FEE + IDX_SELL_BROKER_FEE + IDX_SELL_PPH
)  # compat alias
...
# =============================================================================
# Helper: Defensive DataFrame Conversion & Microstructure Rounding
# =============================================================================
...
class QuantMarketRegimeEngine:
    ENGINE_VERSION: Final[str] = "25.5.2"
...
class QuantProbabilityEngine:
    ...
    def process_calibrated_probabilities(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # PERBAIKAN: Sanitasi ketersediaan kolom raw score
        valid_score_cols = [c for c in self.raw_score_cols if c in df.columns]
        if not valid_score_cols:
            if "raw_score" in df.columns:
                valid_score_cols = ["raw_score"]
            else:
                df = df.with_columns(pl.lit(0.5).alias("raw_score"))
                valid_score_cols = ["raw_score"]

        raw_matrices = df.select(valid_score_cols).to_numpy(order="c").astype(np.float64)
        raw_matrices = np.nan_to_num(raw_matrices, nan=0.0)

        # Penyesuaian bobot dengan kolom yang valid
        if len(valid_score_cols) == len(self.ensemble_weights):
            weights_arr = np.array(self.ensemble_weights, dtype=np.float64)
        else:
            weights_arr = np.ones(len(valid_score_cols), dtype=np.float64) / len(valid_score_cols)

        blended_scores = np.dot(raw_matrices, weights_arr)
...
class QuantExpectedReturnEngine:
    ...
    def process_expected_returns(self, df: pl.DataFrame, trade_size_idr: float = 20_000_000.0) -> pl.DataFrame:
        df = _ensure_polars_df(df)
        t_start = time.perf_counter()
        prob_np = np.nan_to_num(df[self.prob_col].to_numpy().astype(np.float64), nan=np.nan)
...
class QuantConfidenceEngine:
    ...
    def calculate_confidence_scores(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        valid_score_cols = [c for c in self.score_cols if c in df.columns]
        if len(valid_score_cols) > 1:
            scores_np = np.nan_to_num(df.select(valid_score_cols).to_numpy().astype(np.float64), nan=0.0)
...
class QuantExpectedDateEngine:
    ...
    def calculate_expected_horizons(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # PERBAIKAN: Menggunakan pl.String untuk kepatuhan Polars >= 1.0
        out_df = df.with_columns([
            pl.Series(self.output_horizon_col, holding_days, dtype=pl.Float64),
            pl.Series("prediction_horizon", text_horizons, dtype=pl.String)
        ])
...
class QuantCrossSectionalEngine:
    ...
    def rank_universe(self, df: pl.DataFrame) -> pl.DataFrame:
        df = _ensure_polars_df(df)
        t_start = time.perf_counter()
        prob = np.nan_to_num(df["calibrated_prob"].to_numpy().astype(np.float64), nan=np.nan)
        ...
        # PERBAIKAN: Mengganti pl.count() terdepresiasi dengan pl.len()
        ranked_df = temp_df.with_columns([
            (pl.col(self.output_score_col).rank(descending=False) / pl.len()).alias("percentile_rank")
        ])
...
class QuantDriftAndExplainabilityEngine:
    ...
    def process_explainability(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        out_df = df.with_columns([
            ...
            pl.Series("signal_explainability", explainability_list, dtype=pl.String),
...
class QuantGeminiReasoningEngine:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.max_ai_candidates: int = int(self.config.get("max_ai_candidates", 5))
    ...
    def process_qualitative_reasoning(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        conf_multipliers_map: Dict[int, float] = {}

        # PERBAIKAN: Batasi pemanggilan API Gemini hanya untuk kandidat sinyal teratas demi performa & Kuota API
        records = df.to_dicts()
        executable_indices = [
            i for i, r in enumerate(records) if r.get("is_signal_executable", False)
        ]

        # Urutkan berdasarkan skor komposit jika ada, atau kembalikan kandidat utama
        executable_indices.sort(
            key=lambda idx: records[idx].get("composite_quant_score", records[idx].get("calibrated_prob", 0.0)),
            reverse=True
        )

        top_candidates = executable_indices[:self.max_ai_candidates]

        for i, row in enumerate(records):
            if i not in top_candidates:
                if not row.get("is_signal_executable", False):
                    reasoning_map[i] = "Ditolak oleh kuantitatif guardrail."
                else:
                    reasoning_map[i] = "Lolos validasi kuantitatif (Kualitatif AI dilewati)."
                conf_multipliers_map[i] = 1.0
                continue
            ...
            # PERBAIKAN: Fallback bertingkat untuk ketersediaan model Gemini
            reasoning_text = "Analisis AI tidak tersedia."
            for model_target in [PRIMARY_MODEL, FALLBACK_MODEL]:
                try:
                    ...
                    reasoning_text = response.text.strip().replace("\n", " ")
                    break
                except Exception as e:
                    logger.warning(f"⚠️ Error Gemini API ({model_target}) untuk {ticker}: {e}")

            reasoning_map[i] = reasoning_text
            conf_multipliers_map[i] = 1.0

        reasoning_texts = [reasoning_map[i] for i in range(len(records))]
        conf_multipliers = [conf_multipliers_map[i] for i in range(len(records))]

        # PERBAIKAN: Menggunakan pl.String untuk kepatuhan Polars >= 1.0
        out_df = df.with_columns([
            pl.Series("ai_qualitative_reasoning", reasoning_texts, dtype=pl.String),
...
class UnifiedPredictionEngine:
    ...
    """
    ENGINE_VERSION: Final[str] = "2026.Q3.v25.5.2"
    ...
    def run_prediction_pipeline(self, df: Union[pl.DataFrame, Any], total_capital_idr: float = 100_000_000.0, trade_size_idr: float = 20_000_000.0) -> pl.DataFrame:
        df = _ensure_polars_df(df)
        ...
        logger.info(f"Unified Prediction Pipeline v25.5.2 executed for {df_res.height} records in {elapsed_ms:.2f} ms.")
