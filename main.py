...
class ProductionOrchestrator:
    def __init__(self, dry_run: bool = True, self_learning: bool = False, enable_self_evolving: bool = False) -> None:
        ...
        if self.enable_self_evolving and HAS_SELF_EVOLVING_AGENT and GodEntity is not None:
            ...
            logger.info(f"🤖 [AUTONOMOUS_AGENT] self_evolving_agent v{AGENT_VERSION} initialised (safe mode: observe+plan only)")
    ...
    def _step_15_autonomous_agent(self) -> None:
        """Jalankan agen otonom (self_evolving_agent) sebagai lapisan meta-learning."""
        ...
        logger.info("▶ [STEP 15] Autonomous agent observation & planning")
        try:
            ...
            # Observasi lingkungan dan repositori trading
            obs = agent.observe()
            logger.info(f"🔎 [AUTONOMOUS] Observation: files={obs.get('repository', {}).get('files')}, "
                        f"languages={obs.get('repository', {}).get('languages')}")

            # Buat rencana singkat berdasarkan goal trading
            goal = f"Optimize IDX scalping engine configuration and module coordination (execution_id={self.execution_id})"
            agent.set_goal(goal)
            plan = agent.plan(goal)
            logger.info(f"🧠 [AUTONOMOUS] Plan created with {len(plan.get('tasks', []))} tasks")

            # Simpan rencana dan observasi ke memory agen
            ...
            logger.info("✔ [STEP 15] Autonomous agent completed safely (no self-modification)")
        except Exception as e:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self._step_15_autonomous_agent()   # <-- tambahan integrasi self-evolving agent
            logger.info("=" * 60)
            logger.info(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enable-self-evolving", action="store_true",
                        help="Aktifkan autonomous agent (self_evolving_agent) untuk observasi & perencanaan")
