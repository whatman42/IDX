"""
================================================================================
MODULE: signal_idx.py (v2026.5.18 Optimized Sensitivity Edition)
...
class SignalGenerator(_SignalBaseEngine):
    def __init__(self, engine_id: str, operational_config: Dict[str, Any]) -> None:
        super().__init__(engine_id, "v2026.5.18")
    ...
    def generate(self, upstream_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 6. ARA-Aware TP Calculation (Ditingkatkan ke 1.8x ATR agar Net Reward positif menutupi friction)
...
class ExitFilter(_SignalBaseEngine):
    ...
    def filter_exit(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Batas minimum RRR bersih dilonggarkan ke 0.50
...
class ProbabilityFilter(_SignalBaseEngine):
    ...
    def filter_probability(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Ambang batas probabilitas diselaraskan ke 0.50 (netral/cukup berpotensi)
        ...
        # Toleransi EV minimal non-negatif
...
class ConfidenceFilter(_SignalBaseEngine):
    ...
    def filter_confidence(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        cols = input_df.columns
        # Ambang batas konfidensi diselaraskan ke 0.50
...
class SignalExplainer(_SignalBaseEngine):
    ...
    def explain(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        audit_json = pl.struct([
            ...
            pl.lit("v2026.5.18").alias("engine_version")
        ]).struct.json_encode()
...
class UnifiedSignalEngine:
    FACADE_VERSION: Final[str] = "v2026.5.18"
    ...
    def __init__(self, engine_id_prefix: str = "IDXSignalEngine", custom_configs: Optional[Dict[str, Dict[str, Any]]] = None) -> None:
        ...
        # OPTIMIZED CONFIGURATION (HIGH SENSITIVITY & POSITIVE EV GUARANTEE)
        default_cfg = {
            "generator": {
                "min_24h_volume_idr": active_min_adtv,
                "tp_atr_multiplier": 1.8  # TP 1.8x ATR agar Net Reward positif menutupi friction
            },
            "entry_filter": {}, 
            "exit_filter": {
                "min_risk_reward_ratio": 0.50  # Melonggarkan RRR minimal
            }, 
            "prob_filter": {
                "min_prediction_probability": 0.50 # Diselaraskan ke probabilitas 50%
            }, 
            "conf_filter": {
                "min_prediction_confidence": 0.50  # Diselaraskan ke konfidensi 50%
            },
            "tpsl_optimizer": {
