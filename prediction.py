...
Version   : v2026.Q3.v25.4.0 (Institutional Production-Ready Edition)
=============================================================================
"""
...
class FittablePlattCalibrator:
    ...
    def fit_params(self, a: float, b: float) -> None:
        """Updates calibration coefficients dynamically from retraining output."""
        self.a = float(a)
        self.b = float(b)
...
class QuantMarketRegimeEngine:
    """Multi-Factor Institutional Market Regime Engine (Fully Vectorized)."""
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def detect_regimes(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Ensure required columns exist with safe defaults and types
...
class QuantProbabilityEngine:
    """Centralized Institutional Probability Calibration Engine with Downstream Aliases."""
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def process_calibrated_probabilities(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Export calibrated probability along with standard backward-compatible aliases
        out_df = df.with_columns([
...
class QuantExpectedReturnEngine:
    """
    Institutional Expected Return Engine using Model Conditional Expected Return
    and Almgren-Chriss Square-Root Law Market Impact.
    """
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def process_expected_returns(self, df: pl.DataFrame, trade_size_idr: float = 20_000_000.0) -> pl.DataFrame:
        ...
        # Pure Model-Predicted Conditional Upside Return E[R | Signal]
        ...
        # Almgren-Chriss Square-Root Market Impact Law: Impact = sigma * sqrt(Order_Size / Daily_Volume)
        ...
        # Total Realized Execution Friction
...
class QuantConfidenceEngine:
    """Normalized Shannon Entropy Confidence Engine with Guaranteed Prediction Confidence Column."""
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def calculate_confidence_scores(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Normalized Binary Shannon Entropy: H_norm = (-P log2(P) - (1-P) log2(1-P))
        ...
        # Export both signal_confidence and prediction_confidence (solves Gateway 1 missing column warning)
        out_df = df.with_columns([
...
class QuantPriceTargetEngine:
    """Institutional Price Target Engine with IDX Tick Rounding."""
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def calculate_price_targets(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Target = Entry + Expected_Upside * Confidence
...
class QuantStopLossEngine:
    """Dynamic Chandelier & Support Level Stop-Loss Engine with IDX Tick Rounding."""
    ENGINE_VERSION: Final[str] = "25.4.0"
...
class QuantTakeProfitEngine:
    """Take-Profit Engine with Strict RRR Integrity & IDX Tick Rounding."""
    ENGINE_VERSION: Final[str] = "25.4.0"
...
class QuantExpectedDateEngine:
    """Adaptive Holding Period Horizon Engine."""
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def calculate_expected_horizons(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Vectorized Horizon Text Label Allocation
...
class QuantCrossSectionalEngine:
    """
    Cross-Sectional Multi-Factor Ranking Engine with Row-Wise Regime Weights
    and Z-Score Feature Normalization (Fully Vectorized).
    """
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def _z_score_normalize(self, arr: np.ndarray) -> np.ndarray:
        """Z-Score Cross-Sectional Normalization: Z = (X - mean) / (std + eps)."""
        if len(arr) <= 1:
    ...
    def rank_universe(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 1. Z-Score Normalize Features across Universe
        ...
        # 2. Vectorized Weight Array Allocation
...
class QuantPortfolioOptimizerEngine:
    """
    Portfolio Position Sizing using Bayesian Probability Shrinkage Kelly Allocation:
    P_effective = 0.5 + (P_calibrated - 0.5) * Confidence
    """
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def optimize_allocation(self, df: pl.DataFrame, total_capital_idr: float = 100_000_000.0) -> pl.DataFrame:
        ...
        # Bayesian Probability Shrinkage: P_effective = 0.5 + (P_calibrated - 0.5) * Confidence
        effective_prob = 0.5 + ((prob - 0.5) * conf)
        q_effective = 1.0 - effective_prob

        # Uncertainty-Adjusted Kelly: K = (p_eff * b - q_eff) / b
        ...
        # Structural & Mathematical Safety Guardrails
...
class QuantDriftAndExplainabilityEngine:
    """Training Vector Reference Population Stability Index (PSI) & Factor Attribution Engine."""
    ENGINE_VERSION: Final[str] = "25.4.0"
    ...
    def set_training_baseline_vector(self, baseline_prob: np.ndarray) -> None:
        """Stores historical training distribution vector for deterministic PSI calculation."""
    ...
    def _calculate_psi(self, expected: np.ndarray, actual: np.ndarray, bins: int = 10) -> float:
        """Calculates Population Stability Index (PSI) against training reference vector."""
    ...
    def process_explainability(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Compute PSI against training baseline vector if available
        if baseline is not None and len(baseline) > 0:
            psi_score = self._calculate_psi(baseline, prob)
        else:
            psi_score = 0.0

        # Numerical Factor Attribution Calculation
...
# =============================================================================
# 12. UnifiedPredictionEngine (Facade Master Orchestrator)
# =============================================================================
class UnifiedPredictionEngine:
    ...
    Orchestrates total lifecycle and execution sequence across all 11 quantitative sub-engines.
    """
    ENGINE_VERSION: Final[str] = "2026.Q3.v25.4.0"
    ...
    def _validate_required_columns(self, df: pl.DataFrame) -> None:
        """Upfront schema gate validating mandatory streaming dataframe columns."""
        if df.height == 0:
    ...
    def activate_all(self) -> None:
        ...
        logger.info("UnifiedPredictionEngine: All 11 sub-engines ACTIVATED.")
    ...
    def deactivate_all(self) -> None:
        ...
        logger.info("UnifiedPredictionEngine: All 11 sub-engines DEACTIVATED.")

    def set_training_baseline_distribution(self, baseline_prob: np.ndarray) -> None:
        """Passes historical training probability distribution vector for PSI drift tracking."""
    ...
    def run_prediction_pipeline(self, df: pl.DataFrame, total_capital_idr: float = 100_000_000.0, trade_size_idr: float = 20_000_000.0) -> pl.DataFrame:
        ...
        # Step 2: Empirical Probability Calibration (Fittable Platt)
        df_res = self.prob_engine.process_calibrated_probabilities(df_res)

        # Step 3: Expected Return Formulation (Model Conditional Return & Almgren-Chriss Impact)
        df_res = self.er_engine.process_expected_returns(df_res, trade_size_idr=trade_size_idr)

        # Step 4: Signal Confidence Estimation (Normalized Shannon Entropy)
        df_res = self.conf_engine.calculate_confidence_scores(df_res)

        # Step 5: Price Target Projection (Model Upside & IDX Tick Rounded)
        df_res = self.target_engine.calculate_price_targets(df_res)

        # Step 6: Dynamic Chandelier & Support Stop-Loss (IDX Tick Rounded)
        df_res = self.sl_engine.calculate_stop_losses(df_res)

        # Step 7: Take-Profit Validation & RRR Conflict Resolution (IDX Tick Rounded)
        ...
        # Step 9: Cross-Sectional Ranking (Row-Wise Regime Weights & Z-Score Normalization)
        df_res = self.ranking_engine.rank_universe(df_res)

        # Step 10: Portfolio Optimizer (Bayesian Probability Shrinkage Kelly Allocation)
        ...
        logger.info(f"Unified Prediction Pipeline v25.4.0 executed for {df.height} records in {elapsed_ms:.2f} ms.")
    ...
    def shutdown(self) -> None:
        """Gracefully shuts down all 11 sub-engines without throwing AttributeError."""
        with self._state_lock:
        ...
        logger.warning("UnifiedPredictionEngine and all 11 sub-engines have been safely SHUTDOWN.")
