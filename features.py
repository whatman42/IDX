...
Version: 2026.Q3.v17.2-INSTITUTIONAL-HEDGEFUND-GRADE-FIXED
...
try:
    ...
    IDX_FEE_ROUNDTRIP_PCT: Final[float] = 0.004          # 0.40% RT aligned portfolio (0.15% buy + 0.25% sell)
...
# ==============================================================================
# HELPER: VECTORIZED NUMPY KAMA CALCULATION (SAFE FROM NA & ZERO DIVISION)
# ==============================================================================
def _compute_kama_numpy(close_arr: np.ndarray, n: int = 10, fast: int = 2, slow: int = 30) -> np.ndarray:
    ...
    Dioptimalkan secara penuh untuk stabilitas data deret waktu finansial.
    """
    n_obs = len(close_arr)
    ...
    # Inisialisasi awal dengan data bersih
    close_clean = np.nan_to_num(close_arr, nan=0.0)

    # Efficiency Ratio (ER)
    change = np.abs(close_clean[n:] - close_clean[:-n])
    price_diff = np.abs(np.diff(close_clean))
    ...
    # Computation Loop
    kama[n] = close_clean[n]
    for i in range(n + 1, n_obs):
        sc_val = sc[i - n]
        kama[i] = kama[i - 1] + sc_val * (close_clean[i] - kama[i - 1])

    # Forward fill nilai NaN di bagian awal
    if np.isnan(kama[0]):
        first_valid_idx = np.where(~np.isnan(kama))[0]
        if len(first_valid_idx) > 0:
            idx = first_valid_idx[0]
            kama[:idx] = close_clean[:idx]
        else:
            kama[:] = close_clean
...
class IDXTechnicalAndTrendBuilder:
    """Engine Indikator Teknikal, Trend & Adaptive MA (Wilder RSI, KAMA, EWM-trend proxy, Linear Reg Slope)."""
    ...
    def build_all_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Fail-closed: insufficient history / null RSI stays null (NOT neutral 50)
        rsi_expr = pl.when((avg_gain == 0.0) & (avg_loss == 0.0)).then(None).otherwise(rsi_raw)
        ...
        # 3. BOLLINGER BANDS
        bb_mean = pl.col(COL_CLOSE).rolling_mean(window_size=self.bb_window, min_periods=self.bb_window).over(asset_col).forward_fill()
        ...
        # Fail-closed: insufficient BB window → null %B (NOT 0.5 neutral)
        pct_b = ((pl.col(COL_CLOSE) - lower_bb) / (upper_bb - lower_bb + EPSILON))
        ...
        # 5. EWM TREND PROXY (NOT true WMA-based Hull MA — named honestly to avoid HMA claim)
        ewm_half = pl.col(COL_CLOSE).ewm_mean(span=8, adjust=False).over(asset_col)
        ewm_full = pl.col(COL_CLOSE).ewm_mean(span=16, adjust=False).over(asset_col)
        ewm_trend_raw = (2.0 * ewm_half) - ewm_full
        ewm_trend_expr = ewm_trend_raw.ewm_mean(span=4, adjust=False).over(asset_col).forward_fill()

        # 6. KAUFMAN ADAPTIVE MOVING AVERAGE (KAMA 10,2,30) - MAP_GROUPS PERBAIKAN
        ...
        return df_sorted.with_columns([
            ...
            ewm_trend_expr.alias("f_trend_ewm_proxy_16"),
...
class ComprehensiveMicrostructureExtractor:
    """Institutional Multi-Estimator Volatility (Parkinson, Garman-Klass) & Microstructure Engine."""
    ...
    def extract_all(self, df: Union[pl.DataFrame, pl.LazyFrame]) -> Union[pl.DataFrame, pl.LazyFrame]:
        ...
        # 2. ADVANCED VOLATILITY ESTIMATORS (CORRECTED NON-NEGATIVE MATH)
        log_hl = (pl.col(COL_HIGH) / (pl.col(COL_LOW) + EPSILON)).log()
        log_co = (pl.col(COL_CLOSE) / (pl.col(COL_OPEN) + EPSILON)).log()

        # Parkinson Volatility
        parkinson_term = (log_hl ** 2) / (4.0 * math.log(2.0))
        parkinson_vol = (parkinson_term.rolling_mean(window_size=21, min_periods=21).over(asset_col).fill_null(0.0).clip(lower_bound=0.0) * 252.0).sqrt()

        # Garman-Klass Volatility
        gk_term = (0.5 * (log_hl ** 2)) - ((2.0 * math.log(2.0) - 1.0) * (log_co ** 2))
        garman_klass_vol = (gk_term.rolling_mean(window_size=21, min_periods=21).over(asset_col).fill_null(0.0).clip(lower_bound=0.0) * 252.0).sqrt()
...
# ==============================================================================
# SUB-ENGINE 5: FEATURE SELECTION ENGINE (INCREMENTAL PRUNING - DEPRECATION SAFE)
# ==============================================================================
class FeatureSelectionEngine:
    ...
    def fit_transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 2. INCREMENTAL COL-BY-COL PRUNING (REPLACED DEPRECATED FORWARD_FILL)
        asset_col = COL_ASSET if COL_ASSET in df.columns else "ticker"
        selected_set: List[str] = []

        for feat in valid_candidates:
            # Menggunakan Polars Expression fill_null(0.0) modern untuk menghindari Deprecation Warning
            feat_vec = df.select(pl.col(feat).forward_fill().over(asset_col).fill_null(0.0))[feat].to_numpy()
            feat_vec = np.nan_to_num(feat_vec, nan=0.0)
            
            is_collinear = False
            for prev_feat in selected_set:
                prev_vec = df.select(pl.col(prev_feat).forward_fill().over(asset_col).fill_null(0.0))[prev_feat].to_numpy()
...
class UnifiedFeatureEngine:
    ...
    def __init__(self, store_dir: str = "data/feature_store") -> None:
        ...
        logger.info("UnifiedFeatureEngine Saham IDX v2026.Q3.v17.2-INSTITUTIONAL berhasil diinisialisasi.")

    def compute_full_feature_grid(self, df: Union[pl.DataFrame, pl.LazyFrame], run_selection: bool = True) -> pl.DataFrame:
        """
        Pipa Ekstraksi Fitur Terpadu dengan Correct Order of Imputation & Aggressive Garbage Collection.
        """
...
def get_feature_registry() -> Dict[str, FeatureRegistryRecord]:
    ...
    tech_blueprints = [
        ...
        ("f_trend_ewm_proxy_16", "TREND-EWM-PROXY-16", (0.0, None), "Float64"),
