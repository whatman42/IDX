"""
IDX Synchronized Data Engine - Consolidated Quantitative Data Pipeline v2026.Q3.v26.6
=======================================================================================
Production-Grade Scalping Engine & Ultra-Low Overhead Pipeline for Indonesian Stock Exchange (IDX).
Integrates High-Liquidity Filters, Intra-Day Quality Gates, Imputation, Multi-Period Returns,
and Dedicated Quantitative Liquidity Analytics (IDXLiquidityEngine).

Scalping Optimization Highlights:
- Fee Structure: Adjusted Roundtrip Fee to 0.4% (IDX_FEE_ROUNDTRIP_PCT = 0.004) covering Broker, Levy & Tax.
- Liquidity Floor: Raised Minimum Daily Turnover to Rp 5 Billion (IDX_MIN_24H_TURNOVER_IDR = 5,000,000,000.0).
- Staleness Gate: BEI trading-calendar aware (weekend/holiday excluded); wall-clock hard ceiling secondary.
- High-Throughput Ingestion: Optimized batch concurrency & timeout settings for rapid signal generation.
"""
...
# Fast Concurrency Defaults (Optimized for ultra-fast intraday execution)
DEFAULT_MAX_WORKERS: int = 5 if IS_CI_ENV else 4
DEFAULT_BATCH_SIZE: int = 20 if IS_CI_ENV else 15
INTER_BATCH_DELAY_SEC: float = 0.3 if IS_CI_ENV else 0.5
FETCH_TIMEOUT_SEC: float = float(os.getenv("IDX_FETCH_TIMEOUT_SEC", "15.0"))

# ==============================================================================
# OPTIMIZED SCALPING PARAMETERS (BEI / IDX)
# ==============================================================================
IDX_FEE_ROUNDTRIP_PCT: float = 0.004              # 0.4% Total Roundtrip Fee (Broker Buy + Sell + Levy + Tax)
IDX_MIN_PRICE_IDR: float = 50.0                   # IDX Regular Board Floor Price (Rp 50)
IDX_MIN_24H_TURNOVER_IDR: float = 5_000_000_000.0 # Rp 5 Miliar Minimal Daily Turnover (Strict Liquidity for Scalping)
# Wall-clock hard ceiling (safety only). Primary freshness uses BEI trading calendar.
IDX_MAX_HOLIDAY_STALENESS_SEC: float = float(os.getenv("IDX_MAX_STALENESS_SEC", "604800.0"))  # default 7d ceiling
# Max trading-day lag vs last completed BEI session (yfinance daily bars often lag 0–1 day).
IDX_MAX_STALE_TRADING_DAYS: int = int(os.getenv("IDX_MAX_STALE_TRADING_DAYS", "1"))
# Daily bar session close approximation (BEI regular close ~15:50 WIB).
BEI_SESSION_CLOSE_HOUR: int = 16
BEI_SESSION_CLOSE_MINUTE: int = 0

# Official BEI trading holidays (Peng-00171/BEI.POP/09-2025 + adjacent years).
# Weekend is handled separately; dates below are weekday exchange closures.
BEI_HOLIDAYS: frozenset = frozenset({
    # 2025 (subset for year-boundary safety)
    ...
    # 2026 — Pengumuman BEI No. Peng-00171/BEI.POP/09-2025
    ...
    datetime.date(2026, 6, 16),   # 1 Muharam
    datetime.date(2026, 8, 17),   # Proklamasi
    datetime.date(2026, 8, 25),   # Maulid Nabi
    datetime.date(2026, 12, 24),  # Cuti bersama Natal
    datetime.date(2026, 12, 25),  # Natal
    datetime.date(2026, 12, 31),  # Libur Bursa
})


def is_bei_trading_day(d: datetime.date) -> bool:
    """True if BEI is scheduled to trade on calendar date d (Mon–Fri, not holiday)."""
    if d.weekday() >= 5:  # Saturday=5, Sunday=6
        return False
    return d not in BEI_HOLIDAYS


def last_completed_bei_session_date(now: Optional[datetime.datetime] = None) -> datetime.date:
    """Most recent BEI trading day whose regular session is already closed (~16:00 WIB)."""
    now = now or datetime.datetime.now(WIB_TZ)
    if now.tzinfo is None:
        now = now.replace(tzinfo=WIB_TZ)
    else:
        now = now.astimezone(WIB_TZ)
    d = now.date()
    close_today = datetime.datetime(
        d.year, d.month, d.day,
        BEI_SESSION_CLOSE_HOUR, BEI_SESSION_CLOSE_MINUTE,
        tzinfo=WIB_TZ,
    )
    # Before today's close on a trading day → last completed is previous trading day.
    if is_bei_trading_day(d) and now < close_today:
        d = d - datetime.timedelta(days=1)
    # Walk back across weekend / holiday.
    guard = 0
    while not is_bei_trading_day(d) and guard < 30:
        d = d - datetime.timedelta(days=1)
        guard += 1
    return d


def bei_trading_days_between(start: datetime.date, end: datetime.date) -> int:
    """Count of BEI trading days strictly after start up to and including end."""
    if end <= start:
        return 0
    n = 0
    d = start + datetime.timedelta(days=1)
    while d <= end:
        if is_bei_trading_day(d):
            n += 1
...
class IDXCacheManager:
    ...
    def __init__(self, cache_dir: str = ".cache", enable_cache: bool = True,
                 expire_hours: int = 12, encryption_key: str = "") -> None:
        self._enabled = enable_cache
        self._expire_hours = expire_hours  # Reduced TTL to 12 hours for intraday freshness
    ...
    def write_cache(self, cache_key: str, configuration_signature: str, df: pl.DataFrame) -> None:
        ...
        with key_lock:
            try:
                ...
                os.replace(str(temp_meta), str(temp_target))
...
# ==============================================================================
# 2. DATA LOADER (Resilient Fast Scalping Fetcher)
# ==============================================================================
class DataLoader:
    ...
    def __init__(
        ...
        max_staleness_sec: float = IDX_MAX_HOLIDAY_STALENESS_SEC,
        max_stale_trading_days: int = IDX_MAX_STALE_TRADING_DAYS,
    ) -> None:
        self._max_workers = max_workers
        self._lookback_days = lookback_days
        # Absolute wall-clock ceiling only; BEI calendar is the primary freshness gate.
        self._max_staleness_sec = float(max_staleness_sec)
        self._max_stale_trading_days = int(max_stale_trading_days)
        self._backoff_schedule = [2.0, 5.0] if IS_CI_ENV else [5.0, 15.0, 30.0]
    ...
    def fetch_symbol_klines(self, symbol: str, interval: str = "1d", limit: int = 300) -> pl.DataFrame:
        ...
        period_str = "1y" if limit <= 250 else "2y"
        ...
        for attempt in range(max_retries):
            try:
                ...
                pre_delay = random.uniform(0.1, 0.4) if IS_CI_ENV else random.uniform(0.2, 0.6)
                ...
                elif "no data found" in err_str or "delisted" in err_str:
                    logger.warning(f"[DELISTED_TICKER] {clean_symbol} delisted or no data available. Bypassing retries.")
                    break
    ...
    def _validate_freshness_and_clean(self, symbol: str, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if latest_date_str:
            candle_date = datetime.datetime.strptime(str(latest_date_str)[:10], "%Y-%m-%d").date()
            now_wib = datetime.datetime.now(WIB_TZ)
            expected = last_completed_bei_session_date(now_wib)
            lag_trading_days = bei_trading_days_between(candle_date, expected)

            # Primary gate: BEI calendar (weekend/holiday do not count as stale).
            max_lag = getattr(self, "_max_stale_trading_days", IDX_MAX_STALE_TRADING_DAYS)
            if lag_trading_days > max_lag:
                logger.warning(
                    f"[STALE_CANDLE_REJECTED] {symbol}: Latest candle ({candle_date}) "
                    f"lags last completed BEI session ({expected}) by {lag_trading_days} "
                    f"trading day(s) (max_allowed={max_lag})."
                )
                return pl.DataFrame(schema=self._get_schema())

            # Secondary hard ceiling: extreme wall-clock age (default 7 days).
            latest_close = datetime.datetime(
                candle_date.year, candle_date.month, candle_date.day,
                BEI_SESSION_CLOSE_HOUR, BEI_SESSION_CLOSE_MINUTE,
                tzinfo=WIB_TZ,
            )
            staleness_sec = (now_wib - latest_close).total_seconds()
            if staleness_sec > self._max_staleness_sec:
                logger.warning(
                    f"[STALE_CANDLE_REJECTED] {symbol}: Latest candle ({candle_date}) "
                    f"is {staleness_sec / 3600.0:.1f}h old wall-clock "
                    f"(hard ceiling {self._max_staleness_sec / 3600.0:.0f}h)."
                )
                return pl.DataFrame(schema=self._get_schema())

            logger.debug(
                f"[FRESHNESS_OK] {symbol}: candle={candle_date} expected_session={expected} "
                f"lag_trading_days={lag_trading_days}"
            )

        return df_clean

    def fetch_batch_klines(self, symbols: List[str], batch_size: int = DEFAULT_BATCH_SIZE, interval: str = "1d") -> pl.DataFrame:
        ...
        logger.info(
            ...
            f"(max_workers={self._max_workers}, batch_size={batch_size}, timeout={FETCH_TIMEOUT_SEC}s)..."
        )
        ...
        with timer:
            with concurrent.futures.ThreadPoolExecutor(max_workers=self._max_workers) as executor:
                for idx, chunk in enumerate(chunks):
                    ...
                    futures = {executor.submit(self.fetch_symbol_klines, s, interval): s for s in chunk}
...
class IDXCorporateActionsAdjuster:
    ...
    def _get_cached_splits(self, ticker: str) -> Optional[Any]:
        ...
        try:
            global_rate_limiter.wait_if_paused()
            time.sleep(random.uniform(0.2, 0.6) if IS_CI_ENV else 0.5)
...
class IDXMissingDataHandler:
    ...
    def __init__(self, max_missing_ratio: float = 0.15, max_consecutive_missing: int = 2) -> None:
...
# ==============================================================================
# 5. DATA QUALITY GATE (SCALPING STRICT FILTERS)
# ==============================================================================
class IDXDataQualityGate:
    """Enforces multi-layered vectorized validation filters upon IDX market data."""

    def __init__(self, max_stale_bars: int = 5, z_score_threshold: float = 3.5,
                 rolling_window: int = 21) -> None:
        self._max_stale_bars = max_stale_bars  # Tighter stale check for scalping
...
class IDXDataResampler:
    def __init__(self, target_frequency: str = "1d", min_bars_in_window: int = 1) -> None:
...
class IDXReturnsSanitizer:
    def __init__(self, mad_threshold: float = 4.0, rolling_window: int = 14,
                 max_abs_log_return: float = 30.0) -> None:
    ...
    def sanitize_and_compute_returns(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df_returns = df_clean.with_columns([
            ...
            (pl.col("close") / pl.col("close").shift(2).over("ticker") - 1.0).alias("return_2d"),
...
# ==============================================================================
# 9. DEDICATED SCALPING LIQUIDITY ENGINE
# ==============================================================================
class IDXLiquidityEngine:
    """
    Quantitative Liquidity Analytics Engine for Indonesian Stock Exchange (IDX).
    Optimized for Scalping: Filters high-turnover emiten and computes ADTV & RVOL.
    """
    ...
    def filter_dynamic_universe(
        self,
        df: pl.DataFrame,
        min_adtv_20_idr: float = IDX_MIN_24H_TURNOVER_IDR,
        ...
        logger.info(
            f"[DYNAMIC_SCALPING_UNIVERSE] Screened {len(active_universe)} high-liquidity scalping assets "
...
class UniverseGenerator:
    """Static Universe Fallback Engine with High Liquidity Bluechips."""
    ...
    def generate_active_idx_universe(self, output_file: str = "universe.json") -> List[Dict[str, Any]]:
        candidates = [
            ...
            "AMMN.JK", "BREN.JK", "CUAN.JK", "PANI.JK"
        ]
        ...
        try:
            ...
            logger.info(f"[UNIVERSE_GENERATION_SUCCESS] Persisted static scalping universe ({len(active_stocks)} tickers).")
        except Exception as err:
...
class IDXUniverseLoader:
    ...
    def load_active_universe(self) -> List[str]:
        ...
        try:
            ...
            logger.info(f"[UNIVERSE_LOADER] Loaded {len(valid_tickers)} active IDX tickers for Scalping.")
...
# ==============================================================================
# 11. UNIFIED DATA ENGINE FACADE (SCALPING ENGINE)
# ==============================================================================
class UnifiedDataEngine:
    """
    Unified Production Facade for Scalping with Adaptive Rate Limiting,
    Emergency Partial Cache Recovery, and Integrated Liquidity Analytics.
    """
    ENGINE_VERSION: str = "v2026.Q3.v26.6"
    ...
    def __init__(self, universe_file: str = "universe.json", cache_dir: str = ".cache",
        ...
        # Disable CorporateActionsAdjuster in CI/GitHub Actions to save HTTP calls
    ...
    def load_and_prepare_market_data(self, symbols: Optional[List[str]] = None,
        ...
        with timer:
            ...
            cache_key = f"idx_scalping_data_{len(target_symbols)}_{syms_hash}"
            ...
            logger.info(f"[DATA_ENGINE_FETCH] Downloading Scalping OHLCV for {len(target_symbols)} IDX tickers via yfinance...")
            raw_df = self.data_loader.fetch_batch_klines(target_symbols, interval=self.interval)
            ...
            # 6. Quality Gate (Strict Scalping Rules)
            ...
            # 10. Quantitative Scalping Liquidity Analytics Processing
...
def load_and_prepare_market_data(
    ...
    """Load market data with explicit interval propagation.

    Caller-requested interval is forwarded to the provider so internal
    paths do not silently fall back to 1d when another timeframe was asked.
    Default remains "1d" for compatibility with the daily pipeline.
    """
    freq = resample_freq if resample_freq is not None else interval
    engine = UnifiedDataEngine(enable_cache=use_cache, resample_freq=freq, interval=interval)
    return engine.load_and_prepare_market_data(symbols=symbols, use_cache=use_cache)


if __name__ == "__main__":
    logger.info("Initializing IDX Scalping Data Engine Standalone Diagnostic...")
