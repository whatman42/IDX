...
class QuantConformalPredictor:
    ...
    def predict_bounds(self, df: pl.DataFrame, lower_col: str = "conformal_lower", upper_col: str = "conformal_upper") -> pl.DataFrame:
        ...
        dynamic_conf_expr = (pl.col(pred_c) * pl.lit(1.0 - active_q)).clip(0.10, 0.98)
    ...
    def predict(self, df: pl.DataFrame) -> pl.DataFrame:
        if df.height == 0:
            return df.with_columns([
                ...
                pl.lit(0.015).alias("predicted_magnitude"),
        ...
        # PARAMETER TARGET SCALPING INTRADAY BEI (TP: 1.5% - 3.5%, SL: 1.0% - 2.0%)
        volatility_pct = (pl.col(price_col) * pl.lit(0.015)) / (pl.col(price_col) + 1e-8)

        dynamic_tp_pct = (volatility_pct * pl.lit(1.5) + pl.lit(0.012)).clip(0.015, 0.035)  # Target Profit 1.5% - 3.5%
        dynamic_sl_pct = (volatility_pct * pl.lit(1.0) + pl.lit(0.008)).clip(0.010, 0.020)  # Stop Loss 1.0% - 2.0%
        dynamic_holding_days = pl.lit(0)  # Intraday Scalping (Close Position Same Day)
