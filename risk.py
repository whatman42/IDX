"""
IDX Stock Quantitative Trading System - Unified Risk Management Engine v2026.Q3.v3.1.0
Module Name : risk.py
...
# =============================================================================
# KONSTANTA TERKUNCI BURSA EFEK INDONESIA (IDX TRADING BASELINE)
# =============================================================================
IDX_FEE_ROUNDTRIP_PCT: float = 0.003
IDX_MIN_PRICE_IDR: float = 50.0
IDX_MIN_NOTIONAL_IDR: float = 50_000.0
IDX_MIN_24H_VOLUME_IDR: float = 1_000_000_000.0
IDX_MAX_STALENESS_SEC: float = 43200.0
...
class AnomalyDetector:
    ...
    def evaluate(self, df: pl.DataFrame, pipeline_timestamp: str, execution_id: str, trace_id: str = "") -> AnomalyResult:
        ...
        halt = anomaly_score >= 0.75
        warning = (anomaly_score >= 0.40) and not halt
...
class DrawdownGuard:
    ...
    def __init__(
        self,
        max_unrealized_dd: float = 0.15,
        max_realized_dd: float = 0.10,
    ...
    def evaluate(
        ...
        if unrealized_equity is None or unrealized_equity.len() == 0:
            unrealized_equity = pl.Series("unrealized_equity", [100_000_000.0])
        if realized_equity is None or realized_equity.len() == 0:
            realized_equity = pl.Series("realized_equity", [100_000_000.0])
        ...
        warning = ((curr_unreal_dd >= self.max_unrealized_dd * 0.6) or \
                   (curr_real_dd >= self.max_realized_dd * 0.6)) and not halt
...
class LiquidityGuard:
    ...
    def evaluate(self, df: pl.DataFrame, pipeline_timestamp: str, execution_id: str, trace_id: str = "") -> LiquidityResult:
        ...
        if n < self.window_size:
            return LiquidityResult(1.0, 1.0, 1.0, 1.0, True, False, {"status": "INSUFFICIENT_DATA"})
        ...
        halt = (final_liquidity_risk >= 0.75) or (median_value_traded < self.min_value_traded_idr * 0.2)
        warning = (final_liquidity_risk >= 0.45) and not halt
...
class OperationalStateMachine:
    ...
    def _init_sqlite_db(self) -> None:
        with self._lock:
            try:
                # Modus WAL & Synchronous Normal untuk Latensi Ultra-Rendah
    ...
    def evaluate_metrics(self, risk_score: float, psi_score: float, timestamp: str, trace_id: str = "", run_id: str = "", session_id: str = "") -> SystemState:
        with self._lock:
            ...
            elif risk_score >= 0.45 or psi_score > 0.25:
...
class PopulationStabilityIndex:
    ...
    def _compute_single_psi(self, baseline: np.ndarray, actual: np.ndarray) -> float:
        ...
        # Scale-Invariant Offset untuk Baseline Konstan
...
class VolatilityGuard:
    ...
    def evaluate(self, df: pl.DataFrame, pipeline_timestamp: str, execution_id: str, trace_id: str = "") -> VolatilityResult:
        ...
        # Murni Polars Expressions tanpa AttributeError pada Series.log()
        ...
        adaptive_thresh = max(adaptive_thresh, 0.20)
...
class RiskAggregator:
    ...
    def aggregate(
        ...
        final_halt = (len(hard_halt_sources) > 0) or (c_score >= 0.75)
...
class PortfolioRiskExtensions:
    ...
    def check_portfolio_limits(self, market_returns: np.ndarray, asset_returns: np.ndarray) -> Dict[str, Any]:
        ...
        # True Geometric Brownian Motion (GBM) Monte Carlo VaR
        dt = 1.0
...
# =============================================================================
# 11. UNIFIED RISK ENGINE (FACADE CLASS)
# =============================================================================
class UnifiedRiskEngine:
    ...
    def audit_portfolio_orders(self, orders: Any, aggregated_risk: Optional[AggregatedRiskOutput] = None) -> Any:
        """Menyaring order sinyal berdasarkan batasan IDX (Min Price Rp 50, Min Notional Rp 50.000) dengan cached risk output."""
        if orders is None:
            return None
