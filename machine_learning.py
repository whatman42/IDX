...
Version       : 2026.Q3.v16.1 (Institutional Production-Grade & Pipeline Integration Fix)
...
class DataValidationError(Exception):
    """Dilemparkan saat skema atau nilai data melanggar validasi kuantitatif."""
    pass

class ModelExecutionError(Exception):
    """Dilemparkan saat kegagalan runtime optimasi atau inferensi model terjadi."""
    pass

class DatasetBuilderError(Exception):
    """Base exception untuk kegagalan komputasi dataset."""
    pass

class SchemaMismatchedError(DatasetBuilderError):
    """Dilemparkan ketika DataFrame fitur/target tidak memenuhi spesifikasi skema."""
    pass

class ChronologicalViolationError(DatasetBuilderError):
    """Dilemparkan ketika data terdeteksi tidak terurut secara kronologis."""
    pass

class DataLeakageDetectedError(DatasetBuilderError):
    """Dilemparkan jika informasi masa depan bocor ke set pemodelan masa lalu."""
    pass

class EmptyDatasetError(DatasetBuilderError):
    """Dilemparkan ketika hasil penyaringan data menghasilkan sampel di bawah batas aman."""
    pass

class JoinLossExceededError(DatasetBuilderError):
    """Dilemparkan jika kehilangan data akibat join melampaui toleransi."""
    pass

class LightGBMEngineError(ModelExecutionError):
    """Base exception untuk mesin LightGBM."""
    pass

class ModelFactoryError(ModelExecutionError):
    """Base exception untuk operasional Model Factory."""
    pass

class TargetBuilderError(Exception):
    """Base exception untuk kegagalan komputasi pelabelan target."""
    pass

class InvalidPriceSchemaError(TargetBuilderError):
    """Dilemparkan ketika DataFrame harga tidak memenuhi spesifikasi kolom."""
    pass
...
class QuantProbabilityCalibrator:
    ...
    def _defensive_input_gate(self, df: pl.DataFrame, require_target: bool = False) -> None:
        ...
        if self.score_col not in df.columns:
            # Fallback otomatis jika score_col tidak ada, cari probabilitas alternatif
    ...
    def fit(self, df: pl.DataFrame) -> "QuantProbabilityCalibrator":
        ...
        # Binarisasi target jika berbentuk regresi return kontinyu
        y_binary = (y_data > 0.0).astype(int)

        min_samples = int(self.config.get("min_calibration_samples", 20))
        if len(x_data) < min_samples:
            logger.warning(f"Sample Scarcity: Dataset size ({len(x_data)}) < {min_samples}. Skipping strict fit.")
...
class MarketDatasetBuilder:
    ...
    def build_aligned_dataset(
        ...
        valid_features = [c for c in feature_columns if c in features_df.columns]
        ...
        if total_samples < self._min_samples_threshold:
            # Fallback jika sampel kurang strict, ambil data tanpa dropna ketat
...
class MarketTargetBuilder:
    ...
    def build_fixed_horizon_targets(
        ...
        lazy_target = lazy_shifted.with_columns(
            ...
            .otherwise(pl.lit(0.001)) # Fallback imputasi nilai kecil agar tidak null total
            .alias("target")
        )
...
class CatBoostTradingModel:
    ...
    def fit(self, train_df: pl.DataFrame, eval_df: Optional[pl.DataFrame] = None, early_stopping_rounds: int = 10, **kwargs) -> Dict[str, Any]:
        ...
        if cb is not None:
            ...
            # Fallback Linear Regressor jika CatBoost tidak terpasang
    ...
    def predict(self, df: pl.DataFrame) -> np.ndarray:
        ...
        try:
            if cb is not None and hasattr(self._model, "predict"):
                ...
                # Normalisasi output regresi ke probabilitas [0, 1] via sigmoid atau min-max
...
class ModelPredictor:
    ...
    PREDICTOR_VERSION: Final[str] = "2026.Q3.v16.1"
    ...
    def predict(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Dapatkan prediksi probabilitas dari model aktif (CatBoost/XGBoost/DNN)
        try:
        ...
        # Penyesuaian TP/SL Karakteristik Saham IDX (TP: 4% - 22%, SL: 2% - 10%)
...
# ==============================================================================
# 9. UNIFIED MODEL ENGINE (FACADE CLASS WITH FULL PERSISTENCE & PIPELINE FIX)
# ==============================================================================
class UnifiedModelEngine:
    ...
    def prepare_and_fit_pipeline(
        ...
        # Mencegah duplikasi nama kolom saat menggabungkan fitur, target, & meta
        ...
        # Masukkan model hasil latih ke predictor dan calibrator
        self.predictor.set_trained_model(trained_model)
        
        # Fit calibrator & conformal dengan data train
    ...
    def predict_and_calibrate(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Alias kompatibilitas ke prediction.py & validation.py
...
if __name__ == "__main__":
    ...
    n_rows = 150
    dates = [datetime.now(timezone.utc) - timedelta(days=i) for i in range(n_rows)][::-1]
    ...
    # Uji Prediksi, Kalibrasi, & Conformal Bounds
