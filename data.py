"""
=============================================================================
IDX Quantitative Portfolio Engine - Multi-Source Resilient Data Engine
FileName      : data.py
Version       : 2026.Q3.v27.0 (Anti-403 Blocked Failover & Secondary Ingestion)
...
# Deteksi Pustaka Secondary Source (yfinance)
try:
...
class DataSourceBlockedError(DataIngestionError):
    """Penyedia Utama Memblokir Akses (HTTP 403 / Cloudflare WAF)."""
    pass

class RetryableDataSourceError(DataIngestionError):
    """Error Sementara (429, 500, 502, 503, 504, Timeout)."""
    pass

class BEIEmptyResponseError(DataIngestionError):
    """API Mengembalikan Respon Kosong / Invalid."""
    pass
...
def get_previous_trading_days(max_days: int = 5) -> List[Tuple[str, str]]:
    """Trading Calendar Resolver: Menghasilkan tanggal perdagangan aktif (Abaikan Sabtu/Minggu)."""
    trading_days = []
    ...
    while len(trading_days) < max_days:
        ...
        if curr.weekday() < 5:
            ...
            formatted_dash = curr.strftime("%Y-%m-%d")
            trading_days.append((date_str, formatted_dash))
...
def create_idx_http_session() -> requests.Session:
    """Session HTTP dengan Header Lengkap untuk Meniru Browser."""
...
class UnifiedDataEngine:
    """Engine Ingesti Data Pasar Multi-Provider (IDX Primary -> YFinance Secondary -> Local Cache)."""
    ...
    def _fetch_from_idx_primary(self) -> Tuple[List[Dict[str, Any]], str]:
        """Mengambil data dari API Resmi BEI dengan Deteksi Dini HTTP 403."""
        ...
        for date_str, formatted_dash in trading_days:
            ...
            try:
                ...
                if res.status_code == 403:
                    logger.warning(f"🚫 [IDX_PRIMARY_BLOCKED] HTTP 403 Forbidden pada tanggal {date_str} ({req_duration:.1f}ms). IP Diblokir BEI WAF.")
                    raise DataSourceBlockedError("API BEI memblokir akses HTTP Request (403 Forbidden).")
                ...
                logger.warning(f"⚠️ [IDX_PRIMARY_NET_ERR] Gagal terhubung ke BEI: {e}")
    ...
    def _fetch_from_secondary_provider(self, symbols: List[str]) -> pl.DataFrame:
        """Fallback Secondary Ingestion Provider (YFinance Engine)."""
        if not HAS_YFINANCE or not symbols:
            logger.warning("⚠️ Provider sekunder (yfinance) tidak tersedia atau daftar ticker kosong.")
            return pl.DataFrame()

        logger.info(f"🔄 [SECONDARY_FAILOVER] Memulai unduh data via Secondary Provider ({len(symbols)} ticker)...")
        start_time = time.perf_counter()
        
        try:
            yf_symbols = [normalize_idx_symbol(s) for s in symbols]
            data_pd = yf.download(
                tickers=yf_symbols,
                period="5d",
                interval="1d",
            ...
            if data_pd is None or data_pd.empty:
                logger.warning("⚠️ Provider sekunder mengembalikan data kosong.")
                return pl.DataFrame()
    ...
    def load_and_prepare_market_data(
        ...
        # 1. PERIKSA CACHE PARQUET DAHULU
        ...
        symbols_to_fetch = symbols if symbols else ["BBCA.JK", "BBRI.JK", "TLKM.JK"]
        ...
        try:
            ...
            logger.warning(f"🚨 [PRIMARY_BLOCKED_FAILOVER] {blocked_err}. Beralih langsung ke Secondary Provider!")

        # 3. PERCOBAAN SECONDARY SOURCE (JIKA PRIMARY 403 / GAGAL)
        if base_df.is_empty():
            base_df = self._fetch_from_secondary_provider(symbols_to_fetch)

        # 4. TERTIARY FALLBACK: CACHE STALE LOKAL
        if base_df.is_empty() and os.path.exists(cache_file):
            logger.warning("🚨 [OFFLINE_FALLBACK] Seluruh Provider Online Gagal. Menggunakan Offline Parquet Cache...")
            try:
                stale_df = pl.read_parquet(cache_file)
                if stale_df.height > 0:
                    return stale_df, []
            except Exception as e:
                logger.error(f"❌ Offline cache terdistorsi: {e}")

        # 5. TERAKHIR: JIKA SELURUH PROVIDER & CACHE GAGAL TOTAL
        if base_df.is_empty():
        ...
        # 7. KONSOLIDASI DAN PENYIMPANAN CACHE
        if self.enable_cache:
            try:
        ...
        elapsed = time.perf_counter() - start_time
        logger.info(f"⚡ [INGESTION_SUCCESS] Berhasil memuat {full_market_df.height} baris data ({base_df.height} emiten aktif) dalam {elapsed:.2f}s!")
