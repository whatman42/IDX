...
class MarketTargetBuilder:
    ...
    ) -> Dict[str, Tuple[pl.DataFrame, "TargetBuilderTelemetry"]]:
        """Build separate log-return targets for SHORT/MEDIUM/LONG without look-ahead features.

        Horizons (daily bars): SHORT=1, MEDIUM=5, LONG=20.
        Each target uses shift(-h) only as LABEL; features must remain data <= T.
        """
        horizons = horizons or {"SHORT": 1, "MEDIUM": 5, "LONG": 20}
        out: Dict[str, Tuple[pl.DataFrame, TargetBuilderTelemetry]] = {}
        for name, h in horizons.items():
            df_t, tel = self.build_fixed_horizon_targets(price_df, horizon=int(h))
            # tag horizon on target frame for downstream consumers
            if df_t.height and "target" in df_t.columns:
                df_t = df_t.with_columns([
                    pl.lit(str(name)).alias("target_horizon"),
                    pl.lit(float(h)).alias("target_horizon_days"),
                ])
            out[str(name)] = (df_t, tel)
        return out
...
class ModelPredictor:
    ...
    def predict(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        dynamic_holding_days = pl.lit(1)  # SHORT default holding days (daily bars)
