...
Version: 2026.Q3.v17.1-INSTITUTIONAL-HEDGEFUND-GRADE-PATCHED
...
def _compute_kama_numpy(close_arr: np.ndarray, n: int = 10, fast: int = 2, slow: int = 30) -> np.ndarray:
    ...
    Menghindari bug Polars ewm_mean(alpha=Expr) yang memicu TypeError.
    """
    n_obs = len(close_arr)
...
class IDXTechnicalAndTrendBuilder:
    ...
    def build_all_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 6. KAUFMAN ADAPTIVE MOVING AVERAGE (KAMA 10,2,30)
...
class ComprehensiveMicrostructureExtractor:
    ...
    def extract_all(self, df: Union[pl.DataFrame, pl.LazyFrame]) -> Union[pl.DataFrame, pl.LazyFrame]:
        ...
        # 2. ADVANCED VOLATILITY ESTIMATORS (MATH CORRECTION ENFORCED)
        log_hl = (pl.col(COL_HIGH) / (pl.col(COL_LOW) + EPSILON)).log()
        log_co = (pl.col(COL_CLOSE) / (pl.col(COL_OPEN) + EPSILON)).log()

        # Parkinson Volatility: sqrt( 252 * mean( (ln(H/L))^2 / (4 * ln(2)) ) )
        parkinson_term = (log_hl ** 2) / (4.0 * math.log(2.0))
        parkinson_vol = (parkinson_term.rolling_mean(window_size=21, min_periods=21).over(asset_col).fill_null(0.0) * 252.0).sqrt()

        # Garman-Klass Volatility: sqrt( 252 * mean( 0.5 * (ln(H/L))^2 - (2*ln(2) - 1) * (ln(C/O))^2 ) )
...
class FeatureSelectionEngine:
    ...
    def fit_transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 2. INCREMENTAL COL-BY-COL PRUNING (POLARS DEPRECATION-SAFE FORWARD_FILL)
        selected_set: List[str] = []
        for feat in valid_candidates:
            feat_vec = df[feat].forward_fill().fill_null(0.0).to_numpy()
            feat_vec = np.nan_to_num(feat_vec, nan=0.0)
            
            is_collinear = False
            for prev_feat in selected_set:
                prev_vec = df[prev_feat].forward_fill().fill_null(0.0).to_numpy()
...
class UnifiedFeatureEngine:
    ...
    def compute_full_feature_grid(self, df: Union[pl.DataFrame, pl.LazyFrame], run_selection: bool = True) -> pl.DataFrame:
        ...
        # 4. Correct Imputation Sequence (Forward-Fill per asset FIRST, THEN zero-out remaining leading nulls/nans/infs)
