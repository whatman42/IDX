...
# Deteksi Pustaka Secondary Source
try:
...
class BEIEmptyResponseError(DataIngestionError):
    """API Mengembalikan Respon Kosong."""
    pass
...
def get_previous_trading_days(max_days: int = 5) -> List[Tuple[str, str]]:
    """Trading Calendar Resolver: Menghasilkan tanggal perdagangan (Abaikan Sabtu/Minggu)."""
    trading_days = []
...
class UnifiedDataEngine:
    ...
    def _fetch_from_idx_primary((self)) -> Tuple[List[Dict[str, Any]], str]:
        """Mengambil data dari API Resmi BEI dengan Klasifikasi HTTP 403 Serta-merta."""
        ...
        for date_str, formatted_dash in trading_days:
            ...
            try:
                ...
                if res.status_code == 403:
                    logger.warning(f"🚫 [IDX_PRIMARY_BLOCKED] HTTP 403 Forbidden pada tanggal {date_str} ({req_duration:.1f}ms). IP GitHub Runner Diblokir BEI WAF.")
            ...
            if len(yf_symbols) == 1:
                ...
                for sym in yf_symbols:
                    if sym in data_pd.columns.levels[0]:
                        ...
                        if df_sym.empty:
                            continue
                        # Ambil baris terakhir (paling baru)
