"""
================================================================================
MODULE      : signal_idx.py (v2026.Q3.v3.2.2 Institutional Production Edition)
DESCRIPTION : Unified Signal Processing Engine for Indonesian Stock Market (IDX)
              with Adaptive Filter Thresholds & Gemini AI Signal Diagnostics
ARCHITECTURE: 10-Gateway Sequential Pipeline + Gemini AI Diagnostic Engine + Facade Class
DIRECTORY STRUCTURE: Flat Directory (Root Level Integration)
COMPLIANCE  : BEI/IDX Trading Rules, Pure Polars Expressions, Flexible Risk Sizing
================================================================================
"""
...
PRIMARY_MODEL: Final[str] = "gemini-3.6-flash"
FALLBACK_MODEL: Final[str] = "gemini-3.5-flash-lite"
...
# ==============================================================================
# BURSA EFEK INDONESIA (IDX) CONSTANTS (KALIBRASI SENSITIVITAS)
# ==============================================================================
# Fee constants aligned with portfolio.py: buy 0.15% + sell 0.25% (incl. PPh) = 0.40% RT
IDX_BUY_FEE_PCT: Final[float] = 0.0015
IDX_SELL_FEE_PCT: Final[float] = 0.0025
IDX_FEE_ROUNDTRIP_PCT: Final[float] = IDX_BUY_FEE_PCT + IDX_SELL_FEE_PCT  # 0.40%
...
# ADTV floors aligned with portfolio.IDX_MIN_ADTV_IDR (Rp 100 Juta)
IDX_DEFAULT_MIN_24H_VOLUME_IDR: Final[float] = 100_000_000.0  # Standard ADTV (Rp 100 Juta)
IDX_DRYRUN_MIN_24H_VOLUME_IDR: Final[float] = 100_000_000.0   # Dry-run same liquidity floor

IDX_MAX_STALENESS_SEC: Final[float] = 172800.0       # Batas usia data candlestick (48 Jam)
IDX_BASE_OVERNIGHT_GAP_PCT: Final[float] = 0.12    # Toleransi gap overnight 12%
...
def get_prob_expr(cols: List[str]) -> pl.Expr:
    """Helper ekspresi defensif — null stays null (fail-closed, no synthetic prob)."""
    if "prediction_probability" in cols:
        return pl.col("prediction_probability")
    elif "probability" in cols:
        return pl.col("probability")
    return pl.lit(None).cast(pl.Float64)


def get_conf_expr(cols: List[str]) -> pl.Expr:
    """Helper ekspresi defensif untuk konfidensi — null stays null (fail-closed)."""
    if "prediction_confidence" in cols:
        return pl.col("prediction_confidence")
    elif "confidence" in cols:
        return pl.col("confidence")
...
# ==============================================================================
# GEMINI SIGNAL DIAGNOSTIC ENGINE (SUB-ENGINE WITH DUAL-MODEL FALLBACK)
# ==============================================================================
class GeminiSignalDiagnosticEngine:
    """
    Sub-Engine Google Gemini AI sebagai Meta-Diagnostic Layer untuk Evaluasi & Rasionalisasi Sinyal IDX.
    """
    ...
    def __init__(self, api_key: Optional[str] = None) -> None:
        ...
        if HAS_GEMINI_SDK and self.api_key:
            try:
                ...
                logger.info("🤖 [GEMINI_SIGNAL_INIT] Gemini Client terhubung untuk Diagnostik Sinyal IDX.")
            except Exception as err:
                logger.warning(f"⚠️ [GEMINI_SIGNAL_INIT_FAILED] Gagal inisialisasi Gemini Client: {err}")

    def diagnose_signals(self, signal_df: pl.DataFrame) -> Dict[str, Any]:
        """Melakukan analisis kualitatif terhadap sekumpulan sinyal perdagangan kuantitatif IDX."""
        if not self.client or signal_df is None or signal_df.height == 0:
            return {
                "ai_signal_diagnosis": "Gemini AI Client tidak aktif atau sinyal kosong.",
                "quality_assessment": "PASS_THROUGH",
                "recommended_action": "EXECUTE_QUALIFIED_SIGNALS"
            }

        valid_signals = signal_df.filter(pl.col("is_valid_execution").fill_null(False))
        sample_records = valid_signals.head(5).to_dicts() if valid_signals.height > 0 else signal_df.head(5).to_dicts()

        prompt = f"""
        Sebagai Chief Risk Officer & Head of Signal Verification di Broker Saham Bursa Efek Indonesia (BEI/IDX):
        Evaluasi sekumpulan sinyal perdagangan kuantitatif berikut untuk memastikan rasionalitas bisnis, kepatuhan fraksi harga BEI, dan profil Risk/Reward.

        === RINGKASAN BATCH SINYAL ===
        - Total Sinyal Diolah: {signal_df.height}
        - Total Sinyal Lolos Kualifikasi Executable: {valid_signals.height}
        - Sampel Detail Sinyal (Maks 5):
        {json.dumps(sample_records, indent=2, default=str)}

        Tugas:
        1. Berikan evaluasi kualitatif singkat (1-2 kalimat) mengenai keamanan geometri sinyal (Entry, TP, SL, Fraksi Harga, ARA/ARB).
        2. Tentukan Quality Assessment ("HIGH_CONVICTION", "MODERATE_RISK", "SUBSTANDARD").
        3. Tentukan Recommended Action ("PROCEED_EXECUTION", "HALT_FOR_MANUAL_AUDIT", "REJECT_ALL").

        Format Jawaban (JSON murni):
        {{
            "ai_signal_diagnosis": "...",
            "quality_assessment": "HIGH_CONVICTION",
            "recommended_action": "PROCEED_EXECUTION"
        }}
        """
        ...
        for model_target in [PRIMARY_MODEL, FALLBACK_MODEL]:
            try:
                ...
                return {
                    "ai_signal_diagnosis": parsed.get("ai_signal_diagnosis", "Diagnosis sinyal berhasil."),
                    "quality_assessment": parsed.get("quality_assessment", "HIGH_CONVICTION"),
                    "recommended_action": parsed.get("recommended_action", "PROCEED_EXECUTION")
                }
            except Exception as err:
                logger.warning(f"⚠️ [GEMINI_SIGNAL_DIAGNOSIS_WARN] Model {model_target} gagal: {err}")

        return {
            "ai_signal_diagnosis": "Gagal memperoleh respon AI Gemini.",
            "quality_assessment": "MODERATE_RISK",
            "recommended_action": "PROCEED_WITH_CAUTION"
        }


# ==============================================================================
# GATEWAY 1: SIGNAL GENERATOR
# ==============================================================================
class SignalGenerator(_SignalBaseEngine):
    def __init__(self, engine_id: str, operational_config: Dict[str, Any]) -> None:
        super().__init__(engine_id, "v2026.Q3.v3.2.2")
    ...
    def generate(self, upstream_df: pl.DataFrame) -> pl.DataFrame:
        ...
        min_adtv_idr = float(active_cfg.get("min_24h_volume_idr", IDX_DRYRUN_MIN_24H_VOLUME_IDR))
        df_adapted = upstream_df
        cols = list(df_adapted.columns)

        # 1. Safe Auto-Imputation (FAIL-CLOSED — never invent 0.5x probability)
        adapter_exprs = []
        if "prediction_probability" not in cols and "probability" not in cols:
            # Missing score → null → probability/confidence gates reject (NO synthetic BUY)
            adapter_exprs.append(pl.lit(None).cast(pl.Float64).alias("prediction_probability"))
            adapter_exprs.append(pl.lit(None).cast(pl.Float64).alias("probability"))
        ...
        if "prediction_confidence" not in cols and "confidence" not in cols:
            # Fail-closed: missing confidence → null (will fail confidence gate → NO SIGNAL)
            adapter_exprs.append(pl.lit(None).cast(pl.Float64).alias("prediction_confidence"))
            adapter_exprs.append(pl.lit(None).cast(pl.Float64).alias("confidence"))
        ...
        # signal_direction: only default to 1 when an upstream score already exists and is bullish
        if "signal_direction" not in cols:
            if "prediction_probability" in cols or "probability" in cols:
                pcol = "prediction_probability" if "prediction_probability" in cols else "probability"
                adapter_exprs.append(
                    pl.when(pl.col(pcol).is_not_null() & (pl.col(pcol) > 0.50))
                    .then(pl.lit(1))
                    .otherwise(pl.lit(0))
                    .cast(pl.Int8)
                ...
                adapter_exprs.append(pl.lit(0).cast(pl.Int8).alias("signal_direction"))
        ...
        if "horizon" not in cols:
            adapter_exprs.append(pl.lit("SHORT").alias("horizon"))
        if "signal_source" not in cols:
            adapter_exprs.append(pl.lit("ML").alias("signal_source"))
        ...
        # 3. Deduplicate: ticker + horizon (preserve multi-horizon SHORT/MEDIUM/LONG)
        sort_cols = ["prediction_probability"]
        if "horizon" in df_adapted.columns:
            dedupe_subset = [ticker_col, "horizon"]
        else:
            dedupe_subset = [ticker_col]
        df_adapted = df_adapted.sort(by=sort_cols, descending=True, nulls_last=True).unique(
            subset=dedupe_subset, keep="first"
        )
        ...
        risk_dist = entry_ticked - sl_ticked
        reward_dist = tp_ticked - entry_ticked
        rrr_expr = pl.when(risk_dist > 0).then(reward_dist / risk_dist).otherwise(None)
        min_rrr = float(self._config.get("min_risk_reward_ratio", 1.0)) if hasattr(self, "_config") else 1.0
        ...
        reason_expr = (
            ...
            .when(risk_dist <= 0).then(pl.lit("INVALID_RISK_LE_ZERO"))
            .when(rrr_expr.is_null() | (rrr_expr < min_rrr)).then(pl.lit("RRR_BELOW_MINIMUM"))
        ...
        output_df = df_adapted.with_columns([
            ...
            rrr_expr.alias("risk_reward_ratio"),
...
class EntryFilter(_SignalBaseEngine):
    ...
    def filter_entry(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        max_gap_allowed = float(self._config.get("max_overnight_gap_pct", IDX_BASE_OVERNIGHT_GAP_PCT))
        within_gap_limit = gap_pct <= max_gap_allowed
...
class ExitFilter(_SignalBaseEngine):
    ...
    def filter_exit(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        min_rr = float(self._config.get("min_risk_reward_ratio", 0.30))
...
# ==============================================================================
# GATEWAY 4: PROBABILITY FILTER (FLEXIBLE EV AUDIT)
# ==============================================================================
class ProbabilityFilter(_SignalBaseEngine):
    ...
    def filter_probability(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        min_prob = float(self._config.get("min_prediction_probability", 0.48))
        ...
        prob_valid = (p_win >= min_prob) & (ev_idr >= -50.0)
...
class ConfidenceFilter(_SignalBaseEngine):
    ...
    def filter_confidence(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        cols = input_df.columns
        min_conf = float(self._config.get("min_prediction_confidence", 0.48))
...
# ==============================================================================
# GATEWAY 6: TP/SL OPTIMIZER & POSITION SIZING
# ==============================================================================
class TpSlOptimizer(_SignalBaseEngine):
    ...
    def optimize_tp_sl(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        max_risk_pct = float(self._config.get("max_risk_per_trade_pct", 0.02)) 
        max_pos_idr = float(self._config.get("max_position_idr", 2_500_000.0)) 
        ...
        output_df = input_df.with_columns([
            ...
            exact_notional.alias("notional_usdt"),
...
# ==============================================================================
# GATEWAY 7: NUMERICAL SANITY CHECK GATEWAY
# ==============================================================================
...
# ==============================================================================
# GATEWAY 10: SIGNAL EXPLAINER & AUDIT PAYLOAD (POLARS 1.0+ COMPLIANT)
# ==============================================================================
class SignalExplainer(_SignalBaseEngine):
    ...
    def explain(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        safe_entry = pl.col("entry_price").fill_nan(0.0).fill_null(0.0)
        safe_tp = (
            pl.col("optimized_take_profit") if "optimized_take_profit" in input_df.columns
            else pl.col("take_profit") if "take_profit" in input_df.columns
            else pl.lit(0.0)
        ).fill_nan(0.0).fill_null(0.0)
        safe_sl = (
            pl.col("optimized_stop_loss") if "optimized_stop_loss" in input_df.columns
            else pl.col("stop_loss") if "stop_loss" in input_df.columns
            else pl.lit(0.0)
        ).fill_nan(0.0).fill_null(0.0)
        safe_rrr = (
            pl.col("calculated_risk_reward") if "calculated_risk_reward" in input_df.columns
            else pl.col("risk_reward_ratio") if "risk_reward_ratio" in input_df.columns
            else pl.lit(None)
        )
        safe_conf = (
            pl.col("prediction_confidence") if "prediction_confidence" in input_df.columns
            else pl.col("confidence") if "confidence" in input_df.columns
            else pl.lit(None)
        )
        safe_lots = pl.col("lot_quantity").fill_nan(0).fill_null(0).cast(pl.Int64) if "lot_quantity" in input_df.columns else pl.lit(0)
        safe_reason = (
            pl.col("final_validator_reason") if "final_validator_reason" in input_df.columns
            else pl.col("signal_reason") if "signal_reason" in input_df.columns
            else pl.lit("UNKNOWN_REASON")
        ).fill_null("UNKNOWN_REASON")

        is_valid = pl.col("is_valid_execution").fill_null(False) if "is_valid_execution" in input_df.columns else pl.lit(False)
        status = pl.when(is_valid).then(pl.lit(ExplanationStatus.READY)).otherwise(pl.lit(ExplanationStatus.REJECTED))
        side_label = pl.when(is_valid).then(pl.lit("BUY")).otherwise(pl.lit("NO_SIGNAL"))

        # Signal contract detailed reason
        exp_text = pl.concat_str([
            status,
            pl.lit(" "),
            side_label,
            pl.lit(" — "),
            safe_ticker,
            pl.lit(" | Entry: Rp"),
            safe_entry.cast(pl.Int64).cast(pl.String),
            pl.lit(" | TP: Rp"),
            safe_tp.cast(pl.Int64).cast(pl.String),
            pl.lit(" | SL: Rp"),
            safe_sl.cast(pl.Int64).cast(pl.String),
            pl.lit(" | RRR: "),
            safe_rrr.fill_null(0.0).round(2).cast(pl.String),
            pl.lit(" | Confidence: "),
            (safe_conf.fill_null(0.0) * 100.0).round(1).cast(pl.String),
            pl.lit("% | Why: "),
            safe_reason,
        ])
        ...
        audit_json = pl.struct([
            ...
            side_label.alias("side"),
            pl.when(is_valid).then(1.0).otherwise(0.0).alias("signal"),
            safe_entry.alias("entry_price"),
            safe_tp.alias("take_profit"),
            safe_sl.alias("stop_loss"),
            safe_rrr.alias("risk_reward_ratio"),
            safe_conf.alias("confidence"),
            safe_lots.alias("lots"),
            pl.col("notional_idr").fill_nan(0.0).fill_null(0.0).alias("notional_idr") if "notional_idr" in input_df.columns else pl.lit(0.0).alias("notional_idr"),
            safe_reason.alias("reason"),
            pl.lit("v2026.Q3.v3.2.2").alias("engine_version")
        ]).struct.json_encode()

        output_df = input_df.with_columns([
            exp_text.alias("signal_explanation_text"),
            exp_text.alias("signal_explanation"),
...
class UnifiedSignalEngine:
    FACADE_VERSION: Final[str] = "v2026.Q3.v3.2.2"

    def __init__(
        self, 
        engine_id_prefix: str = "IDXSignalEngine", 
        ...
        run_mode = str(os.getenv("RUN_MODE", os.getenv("EXECUTION_MODE", ""))).lower()
        cli_args = " ".join(sys.argv).lower()
        is_dry_run = (
            "dry_run" in run_mode or "dryrun" in run_mode or "reset" in run_mode or
            "simulation" in run_mode or "paper" in run_mode or
            "dry_run" in str(custom_configs).lower() or
            "--dry-run" in cli_args or "--reset-dryrun" in cli_args or "--simulation" in cli_args
        )
        ...
        default_cfg = {
            "generator": {
                ...
                "tp_atr_multiplier": 1.4 if is_dry_run else 1.5
            },
            "entry_filter": {
                "max_overnight_gap_pct": 0.12 if is_dry_run else 0.08
            }, 
            "exit_filter": {
                "min_risk_reward_ratio": 0.30 if is_dry_run else 0.40
            }, 
            "prob_filter": {
                "min_prediction_probability": 0.48 if is_dry_run else 0.50
            }, 
            "conf_filter": {
                "min_prediction_confidence": 0.48 if is_dry_run else 0.50
            },
            "tpsl_optimizer": {
                "portfolio_equity_idr": 10_000_000.0,
                "max_risk_per_trade_pct": 0.02,
                "max_position_idr": 2_500_000.0
            },
            "numerical_sanity": {},
            "ranker": {}, 
            "validator": {}, 
        ...
        self.gemini_engine = GeminiSignalDiagnosticEngine(api_key=gemini_api_key)
        ...
        self.activate_all()
        logger.info(f"UnifiedSignalEngine [{self.engine_id_prefix}] facade initialized (v{self.FACADE_VERSION}) with Gemini AI Diagnostics | Mode ADTV: Rp {active_min_adtv:,.0f} IDR | DryRun: {is_dry_run}.")
    ...
    def execute_pipeline(self, prediction_df: pl.DataFrame, run_ai_diagnostics: bool = True) -> pl.DataFrame:
        ...
        try:
            ...
            if run_ai_diagnostics and final_df.height > 0:
                ai_diag = self.gemini_engine.diagnose_signals(final_df)
                logger.info(f"🤖 [AI_SIGNAL_DIAGNOSIS] Assessment: {ai_diag.get('quality_assessment')} | Action: {ai_diag.get('recommended_action')}")
