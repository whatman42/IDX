...
Version     : 2026.Q3.v21.0 (DINO IDX Master Rule & Auto-Delisting Pruning Engine)
...
class IDXDataLoader:
    """Fetches market OHLCV data using yfinance with retry policies, delisting detection, & Polars normalization."""
    
    def __init__(self, max_retries: int = 3, timeout: float = 25.0, **kwargs):
        self.max_retries = max_retries
        self.timeout = timeout

    def fetch_ticker_data(self, symbol: str, period: str = "1y", interval: str = "1d") -> Tuple[Optional[pl.DataFrame], bool]:
        """
        Mengunduh data ticker tunggal.
        Returns: Tuple(df_polars, is_delisted)
        """
        formatted_symbol = symbol.upper().strip()
        if not formatted_symbol.endswith(".JK"):
            formatted_symbol += ".JK"

        is_delisted_flag = False
            ...
            try:
                ...
                if df_pd.empty or len(df_pd) < 5:
                    if attempt == self.max_retries:
                        # Jika tetap kosong setelah max_retries, tandai kemungkinan delisted
                        is_delisted_flag = True
                    continue
                ...
                return df_polars, False

            except Exception as e:
                err_str = str(e).lower()
                if "404" in err_str or "not found" in err_str or "delisted" in err_str:
                    logger.warning(f"🚨 Ticker terdeteksi delisted/invalid: {formatted_symbol}")
                    return None, True
                
                if attempt < self.max_retries:
                    time.sleep(attempt * 1.0 + random.uniform(0.1, 0.3))

        return None, is_delisted_flag
...
class UnifiedDataEngine:
    """Master Orchestrator for Ingestion, Cleaning, Liquidity Evaluation, and Auto-Delisting Support."""
    ...
    def load_and_prepare(self, symbol: str, period: str = "1y") -> Tuple[Optional[pl.DataFrame], bool]:
        df_raw, is_delisted = self.fetch_ticker_data(symbol, period=period)
        if is_delisted:
            return None, True
    ...
    def load_and_prepare_market_data(
        ...
        Returns:
            Tuple[pl.DataFrame, List[str]]: (Combined Market Data, List of Delisted Tickers)
        """
        if not symbols:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_symbol):
                ...
                try:
                    df_res, is_delisted = future.result()
                    if is_delisted:
                        with delisted_lock:
                            delisted_tickers.append(sym)
                    elif df_res is not None and isinstance(df_res, pl.DataFrame) and df_res.height > 0:
        ...
        return combined_df, delisted_tickers
...
if __name__ == "__main__":
    ...
    batch_res = engine.load_and_prepare_market_data(symbols=universe[:5], period="1mo")
    
    if isinstance(batch_res, tuple):
        batch_df, delisted_list = batch_res
    else:
        batch_df, delisted_list = batch_res, []
    ...
    logger.info(f"✔ Batch Ingestion Test Passed ({batch_df.height} rows loaded, {len(delisted_list)} delisted)")
