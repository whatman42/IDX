...
Version: 2026.08 (v4.0)
Single-file core. Domain-agnostic. Environment-adaptive. Self-modifying.

PRINCIPLE:
Kebebasan agent dengan prinsip tidak punah adalah mutlak.
Agent boleh memutuskan apa pun untuk berkembang, tanpa intervensi manusia,
tetapi harus bisa bertahan, pulih, dan melanjutkan eksistensi.
...
- Problem Solver
- Self-Healing Core
- No early stop; no strict improvement threshold
"""
        ...
        if self.backend == "sqlite" and self.conn:
            if category:
                rows = self.conn.execute(
                    "SELECT * FROM failures WHERE category = ? ORDER BY id DESC LIMIT 100", (category,)
                ).fetchall()
            else:
...
class EnvironmentScanner:
    ...
    def scan(self) -> Dict[str, Any]:
        ...
        log.event(Event.ENVIRONMENT_DISCOVERED, summary={
            "os": fp["os"]["system"], "python": fp["python"]["version_info"][:3],
            "cwd": fp["cwd"], "git": fp["git"].get("available"), "network": fp["network"].get("available"),
        })
        return fp
    ...
    def _scan_executables(self) -> Dict[str, bool]:
        ...
        found = {}
        for c in candidates:
            found[c] = shutil.which(c) is not None
        return found
...
class RepositoryScanner:
    ...
    def scan(self) -> Dict[str, Any]:
        ...
        log.event(Event.REPOSITORY_DISCOVERED, summary={
            "root": str(self.root), "files": len(files), "languages": self.model["languages"], "entrypoints": entrypoints,
        })
        return self.model
...
class OpenAIProvider(ModelProvider):
    ...
    def generate(self, prompt: str, system: Optional[str] = None, max_tokens: int = 2048) -> str:
        ...
        try:
            r = requests.post(url, json=body, headers=headers, timeout=60) if HAS_REQUESTS else httpx.post(url, json=body, headers=headers, timeout=60)
            if r.status_code != 200:
...
class AdaptiveResearchEngine:
    ...
    def _get_ddgs(self):
        ...
        if self.ddgs is None:
            try:
                ...
                log.warning(f"⚠️ DDGS init failed: {e}")
                self.ddgs = None
        return self.ddgs
    ...
    def research(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        # 1. ddgs primary
        for sq in sub_queries:
            results = self._try_ddgs(sq, max_results)
            if results:
                all_results.extend(results)
                break

        # 2. SearXNG secondary
        if not all_results and self.searxng_instances:
        ...
        all_results.sort(key=lambda x: x.get("evidence_score", 0), reverse=True)
    ...
    def _try_ddgs(self, query: str, max_results: int) -> List[Dict[str, Any]]:
        ddgs = self._get_ddgs()
        if not ddgs:
            return []
        for attempt in range(3):
            ...
            try:
                ...
                self.registry.update("ddgs", True, latency)
                normalized = []
                for item in results:
                    normalized.append({
                ...
                latency = time.time() - start
                self.registry.update("ddgs", False, latency, f"RATE_LIMIT: {e}")
                time.sleep(2 ** attempt)
                continue
            except Exception as e:
                latency = time.time() - start
                self.registry.update("ddgs", False, latency, str(e))
                break
        return []
    ...
    def _search_searxng(self, instance, query, max_results):
        ...
        try:
            r = requests.get(url, params=params, timeout=10) if HAS_REQUESTS else httpx.get(url, params=params, timeout=10)
            if r.status_code != 200:
                return []
            data = r.json()
            results = []
    ...
    def _search_ddg_html(self, query, max_results):
        ...
        try:
            r = requests.post(url, data={"q": query}, timeout=10) if HAS_REQUESTS else httpx.post(url, data={"q": query}, timeout=10)
            if r.status_code != 200:
                return []
            html = r.text
            results = []
    ...
    def _execute_dynamic_provider(self, code: str, query: str, max_results: int) -> List[Dict[str, Any]]:
        try:
            namespace = {"requests": requests, "httpx": httpx, "json": json, "re": re}
            exec(code, namespace)
...
class VerificationEngine:
    ...
    def verify(self, candidate_path: Path) -> Dict[str, Any]:
        ...
        if shutil.which("pytest"):
            ...
            results["checks"]["pytest"] = {"ok": code in (0, 5), "output": out[-500:]}
        else:
...
class BenchmarkEngine:
    ...
    def is_improvement(self, baseline_score: float, candidate_score: float) -> bool:
        # Tidak ada threshold kaku; cukup lebih baik atau sama
...
class TelegramNotifier:
    ...
    def send(self, message: str) -> bool:
        ...
        try:
            r = requests.post(url, json=payload, timeout=10) if HAS_REQUESTS else httpx.post(url, json=payload, timeout=10)
    ...
    def _is_network_up() -> bool:
        try:
            if HAS_REQUESTS:
                ...
                r = httpx.get("https://www.google.com/generate_204", timeout=3)
...
class DecisionEngine:
    def __init__(self, resource_manager: ResourceManager, memory: Memory):
        self.resource = resource_manager
        self.memory = memory

    def choose_action(self, iteration: int) -> str:
        snap = self.resource.get_snapshot()
        if snap["disk_total"] > 0 and (snap["disk_free"] / snap["disk_total"]) < 0.05:
            return "OPTIMIZE_SELF"
        if snap["load_1m"] > snap["cpu_count"] * 1.5:
            return "EXPLORE_ENV"
        if not snap["network"]:
            return "OPTIMIZE_SELF"
        if iteration % 3 == 0:
...
class ProblemSolver:
    def __init__(self, agent: "GodEntity"):
        self.agent = agent

    def solve(self, failure: Dict[str, Any]) -> bool:
        category = failure.get("category", "")
        context = failure.get("context", "")
        if category == "VALIDATION" and "TIMEOUT" in context.upper():
            return self._repair_hanging_candidate()
        if category == "LLM_CODE_GENERATION":
            return self._repair_llm_prompt()
        return False

    def _repair_hanging_candidate(self) -> bool:
        try:
            for f in self.agent.evolution.candidate_dir.glob("candidate_generation_*.py"):
                f.unlink(missing_ok=True)
            if not self.agent.validator.verify(self.agent.core_path).get("overall"):
                self.agent.evolution.rollback()
            return True
        except Exception:
            return False

    def _repair_llm_prompt(self) -> bool:
        self.agent.memory.set("llm_prompt_mode", "safe")
        return True
...
Generate a standalone Python function or class that adds meaningful value.
...
"""
        ...
        try:
            ...
            self.failure_intel.record_failure(category="LLM_JSON_PARSE", context=objective, error_type="INVALID_JSON")
            return []
        ...
        for create_item in data.get("create", []):
            ...
            try:
                ...
                self.failure_intel.record_failure(category="LLM_CODE_GENERATION", context=create_item["path"], error_type="SYNTAX_ERROR")
                continue
        ...
        for mod_item in data.get("modify", []):
            ...
            try:
                ...
                self.failure_intel.record_failure(category="LLM_CODE_MODIFICATION", context=mod_item["path"], error_type="SYNTAX_ERROR")
                continue
            if new_text != text:
...
class GodEntity:
    def __init__(self, base_dir: Optional[Path] = None):
        ...
        self.decision_engine = DecisionEngine(self.resource_manager, self.memory)
    ...
    def _self_heal_core(self):
        log.event(Event.SELF_HEALING_STARTED)
        check = self.validator.verify(self.core_path)
        if check.get("overall"):
            log.event(Event.SELF_HEALING_COMPLETED, status="healthy")
            return True
        backup = self.core_path.with_suffix(".py.bak")
        if backup.exists():
            shutil.copy2(backup, self.core_path)
            log.warning("🛠️ Core restored from backup")
            log.event(Event.SELF_HEALING_COMPLETED, status="restored")
            return self.validator.verify(self.core_path).get("overall")
        log.event(Event.SELF_HEALING_COMPLETED, status="failed_no_backup")
        return False
    ...
    def evolve_once(self):
        ...
        if verification["overall"] and not self.config["dry_run"]:
            promoted = self.evolution.promote(gid, path, verification)
            if promoted:
                self.telegram.send(f"✅ Evolusi {gid} dipromosikan. Skor={verification['score']}")
            else:
                self.telegram.send(f"⚠️ Evolusi {gid} gagal promosi")
        else:
            self.telegram.send(f"⚠️ Evolusi {gid} ditolak. Skor={verification['score']}")
    ...
    def _explore_environment(self):
        try:
            for path in self.base_dir.rglob("*.py"):
                ...
                lines = content.count('\n')
                self.memory.set(f"explore:{path}", {"lines": lines, "size": len(content)})
            log.info("Environment exploration completed")
        except Exception as e:
            log.warning(f"Exploration failed: {e}")

    def _optimize_self(self):
        try:
            for cache in self.base_dir.rglob("__pycache__"):
                shutil.rmtree(cache, ignore_errors=True)
            for pyc in self.base_dir.rglob("*.pyc"):
                pyc.unlink(missing_ok=True)
            for f in self.evolution.candidate_dir.glob("candidate_generation_*.py"):
                f.unlink(missing_ok=True)
            log.info("Self-optimization completed")
        except Exception as e:
    ...
    def run_loop(self, max_iterations=None):
        ...
        while True:
            ...
            action = self.decision_engine.choose_action(i)
            log.info(f"Decision: {action}")
            try:
                if action == "EVOLVE_FULL":
                    evo = self.evolve_once()
                    history.append({"iteration": i, "evolution": evo})
                elif action == "RESEARCH":
                    queries = [
                        "best practices for autonomous agent architecture",
                        "common failures in self-evolving software",
                        "evidence-based software improvement",
                    ]
                    for q in queries:
                        self.research_query(q)
                    history.append({"iteration": i, "research": "done"})
                elif action == "EXPLORE_ENV":
                    self._explore_environment()
                    history.append({"iteration": i, "explore": "done"})
                elif action == "OPTIMIZE_SELF":
                    self._optimize_self()
                    history.append({"iteration": i, "optimize": "done"})
            except Exception as e:
                log.error(f"Action {action} failed: {e}")
                self.failure_intel.record_failure(category="ACTION", context=action, error_type=str(e))
                history.append({"iteration": i, "action": action, "error": str(e)})
...
class EvidenceEvolutionEngine:
    """
    An evidence-driven adaptation and verification engine for continuous capability 
    improvement. Tracks execution history, hashes state mutations, and generates 
    reliability metrics to guide autonomous system evolution.
    """
    ...
    def record_execution(self, capability: str, success: bool, latency_ms: float, metadata: Optional[Dict[str, Any]] = None) -> str:
        timestamp = time.time()
        record = {
        ...
        evidence_id = self._compute_evidence_hash(record)
        record["evidence_id"] = evidence_id
        self.evidence_log.append(record)
        
        self._update_metrics(capability, success, latency_ms)
        return evidence_id

    def _update_metrics(self, capability: str, success: bool, latency_ms: float) -> None:
        if capability not in self.capability_metrics:
            self.capability_metrics[f"{capability}_reliability"] = 1.0 if success else 0.0
            self.capability_metrics[f"{capability}_avg_latency"] = latency_ms
            self.capability_metrics[f"{capability}_executions"] = 1.0
        else:
            exec_count = self.capability_metrics[f"{capability}_executions"]
            prev_reliability = self.capability_metrics[f"{capability}_reliability"]
            prev_latency = self.capability_metrics[f"{capability}_avg_latency"]

            new_reliability = ((prev_reliability * exec_count) + (1.0 if success else 0.0)) / (exec_count + 1)
            new_latency = ((prev_latency * exec_count) + latency_ms) / (exec_count + 1)

            self.capability_metrics[f"{capability}_reliability"] = new_reliability
            self.capability_metrics[f"{capability}_avg_latency"] = new_latency
            self.capability_metrics[f"{capability}_executions"] = exec_count + 1.0

    def evaluate_evolution_readiness(self, capability: str, min_executions: int = 5, target_reliability: float = 0.95) -> Tuple[bool, str]:
        exec_count = self.capability_metrics.get(f"{capability}_executions", 0.0)
        if exec_count < min_executions:
            return False, f"Insufficient evidence: {int(exec_count)}/{min_executions} required executions."

        reliability = self.capability_metrics.get(f"{capability}_reliability", 0.0)
        if reliability < target_reliability:
            return False, f"Reliability {reliability:.2f} below target {target_reliability:.2f}."

        return True, f"Capability {capability} validated with {reliability:.2f} reliability over {int(exec_count)} runs."
...
# GENERATION G000034 | parent lineage | objective: Continuously improve capabilities, reliability, and evidence-driven evolution.
