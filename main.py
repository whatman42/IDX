...
VERSION     : 2026.Q3.v2.4.0-SIMULATION-&-SIGNAL-ONLY
...
def create_http_session(
    ...
    """Membangun HTTP Session terpusat untuk keperluan penarikan data & Telegram Webhooks."""
...
class ProductionOrchestrator:
    ...
    def _step_8_risk_management(self) -> None:
        ...
        self.state['validated_orders'] = risk_engine.audit_portfolio_orders(self.state['raw_signals_df'])

    def _step_9_portfolio_execution(self) -> None:
        mode_suffix = _get_mode_suffix()
        state_file = f"portfolio_{mode_suffix}_state.json"
        
        portfolio_engine = UnifiedPortfolioEngine(state_file=state_file, mode=self.config.EXECUTION_MODE)
        ...
        if isinstance(signals_df, pl.DataFrame) and signals_df.height > 0:
            rank_col = next((c for c in ["ranking_score", "score", "probability", "confidence"] if c in signals_df.columns), None)
            if rank_col:
                signals_df = signals_df.sort(rank_col, descending=True)
        ...
        elif hasattr(portfolio_engine, "export_state"):
            portfolio_state = portfolio_engine.export_state()
        elif hasattr(portfolio_engine, "state") and isinstance(portfolio_engine.state, dict):
            portfolio_state = portfolio_engine.state.copy()

        total_eq = getattr(portfolio_engine, "total_equity", portfolio_state.get("total_equity", DEFAULT_DRYRUN_CAPITAL))
        avail_cash = getattr(portfolio_engine, "available_cash", getattr(portfolio_engine, "cash_balance", portfolio_state.get("cash_balance", DEFAULT_DRYRUN_CAPITAL)))
    ...
    def _step_10_execution_layer(self) -> None:
        ...
        if portfolio_engine:
            if hasattr(portfolio_engine, "get_state_dict"):
                ...
                portfolio_state.update(portfolio_engine.state)
    ...
    def _step_16_reporting(self) -> None:
        """Step 16: Centralized Reporting & Telegram Notification Broadcast."""
        try:
