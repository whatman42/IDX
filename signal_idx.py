"""
================================================================================
MODULE      : signal_idx.py (v2026.5.20 Flexible Paper-Trading Adaptive Edition)
...
# Ditingkatkan fleksibilitasnya untuk menangkap saham potensial di mode simulasi
...
IDX_BASE_OVERNIGHT_GAP_PCT: Final[float] = 0.12    # Toleransi gap overnight ditingkatkan ke 12%
...
def get_prob_expr(cols: List[str]) -> pl.Expr:
    ...
    if "prediction_probability" in cols:
        return pl.col("prediction_probability").fill_nan(0.51).fill_null(0.51)
    elif "probability" in cols:
        return pl.col("probability").fill_nan(0.51).fill_null(0.51)
    return pl.lit(0.51)
...
def get_conf_expr(cols: List[str]) -> pl.Expr:
    ...
    if "prediction_confidence" in cols:
        return pl.col("prediction_confidence").fill_nan(0.50).fill_null(0.50)
    elif "confidence" in cols:
        return pl.col("confidence").fill_nan(0.50).fill_null(0.50)
    elif "prediction_probability" in cols:
        return pl.col("prediction_probability").fill_nan(0.50).fill_null(0.50)
    elif "probability" in cols:
        return pl.col("probability").fill_nan(0.50).fill_null(0.50)
    return pl.lit(0.50)
...
# ==============================================================================
# GEMINI SIGNAL DIAGNOSTIC ENGINE (SUB-ENGINE)
# ==============================================================================
class GeminiSignalDiagnosticEngine:
    ...
    def diagnose_signals(self, signal_df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"⚠️ [GEMINI_SIGNAL_DIAGNOSIS_FAILED] Error saat memanggil Gemini API: {err}")
            return {
...
class SignalGenerator(_SignalBaseEngine):
    def __init__(self, engine_id: str, operational_config: Dict[str, Any]) -> None:
        super().__init__(engine_id, "v2026.5.20")
    ...
    def generate(self, upstream_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 1. Safe Auto-Imputation (Impute ke 0.51)
        adapter_exprs = []
        ...
        if "prediction_confidence" not in cols and "confidence" not in cols:
            adapter_exprs.append(pl.lit(0.50).cast(pl.Float64).alias("prediction_confidence"))
            adapter_exprs.append(pl.lit(0.50).cast(pl.Float64).alias("confidence"))
        ...
        # 6. ARA-Aware TP Calculation (Disesuaikan ke 1.5x ATR)
...
class ExitFilter(_SignalBaseEngine):
    ...
    def filter_exit(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Batas minimum RRR bersih dilonggarkan ke 0.30 untuk Paper Trading
...
class ProbabilityFilter(_SignalBaseEngine):
    ...
    def filter_probability(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Ambang batas probabilitas diselaraskan ke 0.48
        ...
        # Toleransi EV minimal fleksibel
...
class ConfidenceFilter(_SignalBaseEngine):
    ...
    def filter_confidence(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        cols = input_df.columns
        # Ambang batas konfidensi diselaraskan ke 0.48
...
# ==============================================================================
# GATEWAY 10: SIGNAL EXPLAINER & AUDIT PAYLOAD
# ==============================================================================
class SignalExplainer(_SignalBaseEngine):
    ...
    def explain(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        audit_json = pl.struct([
            ...
            pl.lit("v2026.5.20").alias("engine_version")
        ]).struct.json_encode()
...
class UnifiedSignalEngine:
    FACADE_VERSION: Final[str] = "v2026.5.20"
    ...
    def __init__(
        ...
        # OPTIMIZED CONFIGURATION (HIGH SENSITIVITY FOR PAPER TRADING & SIMULATION)
        default_cfg = {
            "generator": {
