...
VERSION     : v2026.Q3.v3.1.0-QUOTA-RESILIENT-AUTOPILOT
PYTHON VER  : 3.10+ / 3.11+ / 3.12+
COMPLIANCE  : DINO IDX Master Rules, Dynamic Universe Sync, Pure Quant Fail-Safe
================================================================================
"""
...
UNIVERSE_FILE: Final[str] = "universe.json"
...
# =============================================================================
# DEFAULT TICKER UNIVERSE & BEI CONSTANTS (FALLBACK BASELINE)
# =============================================================================
DEFAULT_IDX_TICKERS: Final[List[str]] = [
    ...
    "AUTO.JK", "BABP.JK", "BACA.JK", "BALI.JK", "BAPA.JK", "BAPI.JK", "BATA.JK", "BBCA.JK", "BBHI.JK",
    ...
    "BTPS.JK", "BUDI.JK", "BUVA.JK", "BYAN.JK", "CAKK.JK", "CAMP.JK", "CANI.JK", "CARS.JK", "CASA.JK",
    ...
    "EPAC.JK", "ERAA.JK", "ESSA.JK", "ESTA.JK", "FAST.JK", "FASW.JK", "FILM.JK", "FISH.JK", "FITT.JK",
    "FLMC.JK", "FMII.JK", "FOOD.JK", "FORU.JK", "FPNI.JK", "GAMA.JK", "GEMS.JK", "GGRM.JK", "GJTL.JK", "GLVA.JK",
    "GOOD.JK", "GPRA.JK", "GSMF.JK", "GZCO.JK", "HERO.JK", "HEXA.JK", "HITS.JK", "HKMU.JK", "HMSP.JK",
    ...
    "LEAD.JK", "LIFE.JK", "LINK.JK", "LPKR.JK", "LPPF.JK", "LUCK.JK", "MAIN.JK", "MAPB.JK", "MAPA.JK",
    "MCAS.JK", "MDKA.JK", "MEDC.JK", "MIDI.JK", "MIRA.JK", "MITI.JK", "MKNT.JK", "MLPL.JK", "MLPT.JK",
    ...
    "TCID.JK", "TINS.JK", "TKIM.JK", "TLKM.JK", "TOTO.JK", "TPIA.JK", "TRIM.JK", "ULTJ.JK", "UNIC.JK",
    "UNTR.JK", "UNVR.JK", "WIKA.JK", "WSBP.JK", "WSKT.JK", "YPAS.JK", "ZINC.JK"
]

DEFAULT_SIGNAL_HORIZONS: Final[List[int]] = [1, 3, 5, 10, 20, 60]
DEFAULT_DRYRUN_CAPITAL: Final[float] = 10_000_000.0


# =============================================================================
# DYNAMIC UNIVERSE MANAGER (AUTO-IPO & AUTO-DELISTING PRUNING)
# =============================================================================
def fetch_latest_idx_active_tickers(logger: logging.Logger) -> List[str]:
    """Mengambil daftar seluruh saham aktif langsung dari API Publik BEI (IDX)."""
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            tickers = [f"{p['KodeEmiten'].strip().upper()}.JK" for p in profiles if p.get("KodeEmiten")]
            if tickers:
                logger.info(f"🌐 [IDX_API_SUCCESS] Berhasil sinkronisasi {len(tickers)} emiten aktif dari BEI.")
                return sorted(list(set(tickers)))
    except Exception as e:
        logger.warning(f"⚠️ API IDX tidak merespon: {e}. Menggunakan fallback data lokal.")
    
    return []


def sync_universe_file(logger: logging.Logger, delisted_tickers: Optional[List[str]] = None) -> List[str]:
    """
    Memperbarui universe.json secara otomatis:
    1. Memuat emiten dari universe.json lokal.
    2. Menambahkan saham IPO baru hasil sinkronisasi API BEI.
    3. Mencoret ticker delisted dari daftar universe.
    """
    ...
    if os.path.exists(UNIVERSE_FILE):
        try:
            with open(UNIVERSE_FILE, "r", encoding="utf-8") as f:
                payload = json.load(f)
                existing_tickers = payload.get("symbols", [])
        except Exception as e:
            logger.warning(f"⚠️ Gagal membaca {UNIVERSE_FILE}: {e}")

    fetched_tickers = fetch_latest_idx_active_tickers(logger)
    combined_set = set(existing_tickers).union(set(fetched_tickers))
    
    if not combined_set:
        combined_set = set([normalize_idx_symbol(t) for t in DEFAULT_IDX_TICKERS])

    if delisted_tickers:
        for bad_ticker in delisted_tickers:
            norm_bad = normalize_idx_symbol(bad_ticker)
            if norm_bad in combined_set:
                combined_set.remove(norm_bad)
                logger.info(f"🗑️ [AUTO_PRUNE] Ticker delisted dicoret dari universe: {norm_bad}")

    final_tickers = sorted(list(combined_set))

    universe_payload = {
        "updated_at": datetime.now(WIB_TZ).isoformat(),
        "total_tickers": len(final_tickers),
        "symbols": final_tickers
    }

    try:
        with open(UNIVERSE_FILE, "w", encoding="utf-8") as f:
            json.dump(universe_payload, f, indent=2)
        logger.info(f"💾 [UNIVERSE_SYNCED] Berkas {UNIVERSE_FILE} tersimpan. Total Emiten Aktif: {len(final_tickers)}")
    except Exception as e:
        logger.error(f"❌ Gagal menyimpan {UNIVERSE_FILE}: {e}")
...
# =============================================================================
TOP_CANDIDATES_SCHEMA: Final[Dict[str, Any]] = {
    "type": "OBJECT",
    "properties": {
        "analyses": {
            "type": "ARRAY",
...
class IDXGeminiInsightEngine:
    """Engine integrasi Google Gemini untuk kelola parameter dinamis & 2-Tier Deep Dive dengan Quota Fail-Safe."""
    ...
    def verify_health(self) -> Tuple[bool, Dict[str, str]]:
        """Menjalankan Gemini Model Validation & Health Check (Resilient against Quota Exhaustion)."""
        results = {
            "env_api_key": "FAIL",
            "gemini_primary": "FAIL",
        ...
        for model_name, key in [(PRIMARY_MODEL, "gemini_primary"), (FALLBACK_MODEL, "gemini_fallback")]:
            try:
                ...
                if "429" in err_str or "quota" in err_str or "resourceexhausted" in err_str:
                    self.logger.warning(f"🚨 [GEMINI_QUOTA_EXHAUSTED] Kuota habis pada model '{model_name}': {e}")
                else:
    ...
    def get_dynamic_trading_parameters(self) -> Dict[str, Any]:
        """Gemini Administrator mengatur konstanta & syarat nominal fleksibel (Sesuai Maximum Safe Relaxation)."""
        defaults = {
            "min_adtv_idr": 50_000_000.0,
            "min_confidence": 0.40,
            "min_rrr": 1.1,
            "max_concurrent_positions": 5,
            "risk_scale": 0.5,
            "configured_by": "DEFAULT_MAX_RELAXATION_PROTOCOL"
        }
        ...
        try:
            ...
            self.logger.warning(f"⚠️ Gagal memuat konfigurasi otomatis Gemini (Quota/Error), memakai default relaxation: {e}")
    ...
    def analyze_top_candidates(
        ...
        Sistem kuantitatif memerlukan evaluasi cepat pada kandidat saham.
        Misi Anda: LONGGARKAN kriteria fundamental & konfirmasi teknikal.
        - Turunkan standar confidence minimal menjadi 0.40.
...
"""
        ...
        for target_model in models_to_try:
            ...
            for attempt in range(1, 3):
                try:
                    self.logger.info(f"[GEMINI_DEEP_DIVE] Menganalisis Top 5 Kandidat via '{target_model}' [Relaxed={relaxation_mode}] [Attempt {attempt}/2]...")
                    ...
                    if response and hasattr(response, "text") and response.text:
                        ...
                        data_res = json.loads(cleaned)
                        results_by_ticker: Dict[str, Dict[str, Any]] = {}

                        for item in data_res.get("analyses", []):
                    ...
                    if "429" in err_str or "quota" in err_str or "resourceexhausted" in err_str:
                        self.logger.warning(f"🚨 [QUOTA_EXHAUSTED] Gemini API kuota habis pada model '{target_model}'. Dialihkan ke Pure Quant Mode.")
                        return {}
...
class OrchestratorConfig:
    ...
    IDX_TICKERS: List[str] = field(default_factory=list)
...
def reset_dryrun_balance(logger: logging.Logger) -> None:
    ...
    state_files = ["portfolio_simulation_state.json", "portfolio_dryrun_state.json", "portfolio_state.json"]
...
class ProductionOrchestrator:
    ...
    def execute_required_health_checks(self) -> bool:
        """Health Check teruji: Jika Gemini gagal/kuota habis, bot beralih ke Pure Quant Mode tanpa crash."""
        self.logger.info("==================================================")
        self.logger.info("🔍 STARTING PRE-FLIGHT HEALTH CHECKS")
        self.logger.info("==================================================")
        
        health_status = {}

        # 1. Check Data Source & Universe File
        ...
        if health_status["DATA_SOURCE"] == "FAIL":
            self.logger.critical("🛑 [CRITICAL_FAIL] Data Source / Universe tidak ditemukan!")
            return False

        # 2. Check Gemini AI Availability & Quota
        ...
        if not gemini_ok:
            self.logger.warning("⚠️ [GEMINI_UNAVAILABLE / QUOTA_EXHAUSTED] Gemini API offline atau kuota habis.")
            self.logger.warning("🛡️ [SYSTEM_AUTOPILOT] Mengaktifkan PURE QUANTITATIVE FALLBACK MODE (100% Python Engine).")
            self.state['gemini_available'] = False
        else:
            self.logger.info("✅ [GEMINI_AVAILABLE] Gemini AI aktif normal.")
            self.state['gemini_available'] = True
        ...
        self.logger.info(f"[MARKET TIME] (Asia/Jakarta) ..... {now_wib.strftime('%Y-%m-%d %H:%M:%S %Z')}")
        self.logger.info("==================================================")
    ...
    def run(self) -> None:
        ...
        try:
            with StepContext(1, "Bootstrap & Dynamic Universe Validation", self.logger, critical=True):
    ...
    def _step_1_bootstrap(self) -> None:
        ...
        # OTOMATISASI UNIVERSE: Sinkronisasi IPO & Delisting
        active_tickers = sync_universe_file(self.logger)
        self.config.IDX_TICKERS = [normalize_idx_symbol(t) for t in active_tickers if t]

        if self.state.get('gemini_available', True):
            ...
            gemini_params = {
                ...
                "max_concurrent_positions": 5,
                "risk_scale": 0.5,
                "configured_by": "PURE_QUANT_FALLBACK"
            }
    ...
    def _step_2_data_layer(self) -> None:
        ...
        try:
            ...
            ingest_result = data_engine.load_and_prepare_market_data(
            ...
            if isinstance(ingest_result, tuple):
                market_data, delisted_found = ingest_result
            else:
                market_data = ingest_result

            if delisted_found:
                self.logger.info(f"🧹 Delisting terdeteksi pada {len(delisted_found)} emiten. Menghapus dari universe...")
                updated_tickers = sync_universe_file(self.logger, delisted_tickers=delisted_found)
                self.config.IDX_TICKERS = updated_tickers
    ...
    def _step_6_signal_engine(self) -> None:
        """
        Step 6: 2-Tier Signal Engine dengan Proteksi Kuota Gemini Habis (Pure Quant Fallback).
        """
        ...
        # ---------------------------------------------------------------------
        # TIER 2: GEMINI QUALITATIVE ANALYSIS (Hanya jika API Aktif & Kuota Ada)
        # ---------------------------------------------------------------------
        ai_results = {}
        gemini_is_available = self.state.get('gemini_available', True)

        if gemini_is_available:
            try:
                self.logger.info("🧠 [TIER_2_GEMINI] Mengirim Top 5 Kandidat ke Gemini AI untuk analisis...")
                ...
                if not has_buy_signal and len(ai_results) > 0:
                    self.logger.info("🔓 [ADAPTIVE_LOOSENING] Memicu pelonggaran syarat otomatis...")
                    ai_results = self.gemini_engine.analyze_top_candidates(top_5_report, relaxation_mode=True)
            except Exception as e:
                self.logger.warning(f"⚠️ Terjadi kesalahan pada Gemini API (Quota Exhausted/Network): {e}")
                ai_results = {}

        # ---------------------------------------------------------------------
        # PURE QUANT FALLBACK: JIKA GEMINI OFFLINE / KUOTA HABIS
        # ---------------------------------------------------------------------
        use_quant_fallback = (len(ai_results) == 0)
        if use_quant_fallback:
            self.logger.warning("🚨 [PURE_QUANT_FALLBACK] Mengaktifkan Eksekusi Sinyal Murni Python ML!")

        final_signal_rows = []
        min_conf = self.state.get('gemini_trading_params', {}).get('min_confidence', 0.40)

        for idx, row in enumerate(top_5_dicts):
            ticker = str(row.get(asset_col, "")).upper().strip()
            
            if use_quant_fallback:
                raw_prob = float(row.get(prob_col, 0.50)) if prob_col else 0.50
                
                # Aturan Keputusan Murni Python
                if raw_prob >= (min_conf + 0.10):
                    signal_dec = "BUY"
                elif raw_prob >= min_conf:
                    signal_dec = "BUY" if idx == 0 else "HOLD"  # Utamakan Peringkat 1 untuk BUY
                else:
                    signal_dec = "NO_SIGNAL"

                row["signal"] = signal_dec
                row["signal_candidate"] = signal_dec
                row["confidence"] = raw_prob
                row["fundamental_score"] = 50.0
                row["ai_reason"] = f"PURE_QUANT_MODE (ML Prob: {raw_prob:.2f})"
    ...
    def _step_7_validation(self) -> None:
        ...
        try:
            self.state['val_results'] = val_engine.validate_full_pipeline(
                ...
                run_ai_diagnostics=self.state.get('gemini_available', True)
            )
        except TypeError:
            self.state['val_results'] = val_engine.validate_full_pipeline(
                df=self.state['raw_signals_df'], 
    ...
    def _step_9_portfolio_execution(self) -> None:
        ...
        processed_signals = portfolio_engine.process_trading_signals(signals_df, latest_prices, top_n=1)
    ...
    def _step_16_reporting(self) -> None:
        """Step 16: Reporting & Telegram Broadcast dengan Fallback Narasi Murni."""
        try:
            ...
            if ai_narrative:
                ...
                portfolio_state["ai_insight"] = "Eksekusi Sistem Kuantitatif Murni (Pure Quant Mode Active)."
...
if __name__ == "__main__":
    ...
    if "--bootstrap-universe" in sys.argv:
        sys.stdout.write("⚙️ [BOOTSTRAP] Memulai generasi & sinkronisasi universe.json...\n")
        try:
            temp_logger = logging.getLogger("IDX.Bootstrap")
            synced_symbols = sync_universe_file(temp_logger)
            sys.stdout.write(f"✔ [BOOTSTRAP_SUCCESS] Berhasil menulis {len(synced_symbols)} ticker ke {UNIVERSE_FILE}\n")
            sys.exit(0)
