...
FileName      : data.py
Version       : 2026.Q3.v27.1 (Syntax Clean & Non-Retryable Failover Engine)
...
def get_previous_trading_days(max_days: int = 5) -> List[Tuple[str, str]]:
    ...
    while len(trading_days) < max_days:
        if curr.weekday() < 5:  # 0-4 = Senin-Jumat
...
class UnifiedDataEngine:
    ...
    # --------------------------------------------------------------------------
    # SOURCE 1: PRIMARY BEI DIRECT API (Fix Syntax & Non-Retryable Check)
    # --------------------------------------------------------------------------
    def _fetch_from_idx_primary(self) -> Tuple[List[Dict[str, Any]], str]:
        """Mengambil data dari API BEI dengan Klasifikasi HTTP 403 Serta-merta."""
        ...
        for date_str, formatted_dash in trading_days:
            ...
            try:
                ...
                # 🛑 NON-RETRYABLE ERROR: HTTP 403 (BLOCKED BY WAF)
                ...
                if res.status_code in RETRYABLE_HTTP_STATUS:
                    logger.warning(f"⚠️ [IDX_HTTP_TRANSIENT] Status HTTP {res.status_code} pada tanggal {date_str}. Menjajal tanggal berikutnya...")
                    continue
    ...
    def load_and_prepare_market_data(
        ...
        # 6. SINTESIS DERET WAKTU HISTORIS
