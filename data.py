"""
IDX Synchronized Data Engine - Consolidated Quantitative Data Pipeline v2026.8.3
=======================================================================================
...
Refactoring & Fixes Highlights (v2026.8.3 - Fast-CI Patch):
- Fix 1: High-Throughput Batch Concurrency in CI (workers=5, batch_size=20) to fix CI timeout.
- Fix 2: Fail-Fast Backoff Schedule for CI (2s -> 5s max) preventing stuck jobs on delisted tickers.
- Fix 3: Reduced pre-request jitter delay in CI (0.2s - 0.8s).
- Fix 4: Immediate cancellation on "No data found / delisted" errors.
- Fix 5: Retained all v2026.8.2 Features (IDXLiquidityEngine, Parquet Cache, Main Alignment).
"""
...
# Fast Concurrency Defaults (Optimized to complete 300+ tickers under 2 minutes in CI)
DEFAULT_MAX_WORKERS: int = 5 if IS_CI_ENV else 4
DEFAULT_BATCH_SIZE: int = 20 if IS_CI_ENV else 15
INTER_BATCH_DELAY_SEC: float = 0.5 if IS_CI_ENV else 0.5
...
# ==============================================================================
# 2. DATA LOADER (Resilient Fast Fetcher)
# ==============================================================================
class DataLoader:
    ...
    def __init__(self, base_url: Optional[str] = None, max_workers: int = DEFAULT_MAX_WORKERS,
        ...
        # Fail-fast backoff schedule in CI environment to avoid wasting minutes on dead/delisted tickers
        self._backoff_schedule = [2.0, 5.0] if IS_CI_ENV else [30.0, 60.0, 120.0, 240.0, 300.0]
    ...
    def fetch_symbol_klines(self, symbol: str, interval: str = "1d", limit: int = 300) -> pl.DataFrame:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Optimized pre-request jitter delay for fast throughput
                pre_delay = random.uniform(0.2, 0.8) if IS_CI_ENV else random.uniform(0.5, 1.5)
                ...
                if hist_df is None or hist_df.empty:
                    # Skip retries if empty response implies no data / delisted
                    break
...
class IDXCorporateActionsAdjuster:
    ...
    def _get_cached_splits(self, ticker: str) -> Optional[Any]:
        ...
        try:
            global_rate_limiter.wait_if_paused()
            time.sleep(random.uniform(0.5, 1.5) if IS_CI_ENV else 0.5)
