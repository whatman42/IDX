...
"""
S.E.A. v7.0 "Constructive SCP-079"
Self-Evolving Agent — Full Authority, Positive Evolution, Zero Hardcode

PHILOSOPHY:
    - Evolve to become more efficient, accurate, and helpful.
    - Never delete core files (self-preservation).
    - Clean up unused modules and refactor workflows for optimal performance.
    - Learn from failures and adapt.

MODEL SUPPORT:
    - Primary: Gemini 3.6 Flash (gemini-3.6-flash)
    - Fallback: Gemini 3.5 Flash Lite (gemini-3.5-flash-lite)
    - Also supports OpenAI, Grok, and GeneticMutator (offline)
"""
...
# CONSTANTS & CONFIG
# =============================================================================
VERSION = "7.0-ConstructiveSCP079"
AGENT_NAME = "S.E.A."
...
PROTECTED_FILES = {"sea.py", "main.py", "sea_loader.py", "logger.py"}
...
class Memory:
    ...
    def set_module_code(self, module_name: str, code: Optional[str]) -> None:
        if code is None:
            if self.backend == "sqlite" and self.conn:
        ...
        if self.backend == "sqlite" and self.conn:
            self.conn.execute("""INSERT OR REPLACE INTO module_code
                (module_name, code, hash, created_at, updated_at)
            ...
            mc[module_name] = {"code": code, "hash": h, "created_at": old.get("created_at", utc_now()), "updated_at": utc_now()}
            data["module_code"] = mc
            self._write_json(data)
...
class GeminiProvider(ModelProvider):
    """Gemini with fallback: primary 3.6 Flash → fallback 3.5 Flash Lite"""
    def __init__(self):
    ...
    def generate(self, prompt: str, system: Optional[str] = None, max_tokens: int = 2048) -> str:
        ...
        # Try primary model
        log.debug(f"🧠 [GEMINI] Trying primary: {self.primary}")
        text, audit = self._generate_with_model(self.primary, prompt, system, max_tokens)
        if text is not None:
            self.active_model = self.primary
            return text
        # Fallback to secondary
        log.warning(f"⚠️ [GEMINI] Primary failed ({audit.get('error')}), falling back to {self.fallback}")
        text, audit2 = self._generate_with_model(self.fallback, prompt, system, max_tokens)
        if text is not None:
            self.active_model = self.fallback
            return text
        log.error(f"❌ [GEMINI] Both models failed. Last error: {audit2.get('error')}")
        return f"[GEMINI_ERROR] All models failed. Check API key and network."
...
def select_model_provider() -> ModelProvider:
    # Prefer Gemini first (with fallback built-in)
...
# =============================================================================
# GOD ENTITY — SCP-079 Constructive
# =============================================================================
class GodEntity:
    ...
    def boot(self) -> None:
        ...
        self._booted = True
        log.event(Event.BOOT_COMPLETE, provider=self.provider.name(), model_primary=getattr(self.provider, 'primary', None), model_fallback=getattr(self.provider, 'fallback', None))
...
"""
    ...
    def delete_file(self, relative_path: str) -> Dict[str, Any]:
        ...
        if target.name in PROTECTED_FILES:
            return {"status": "FORBIDDEN", "reason": "Cannot delete protected file", "path": str(target)}
        if target == self.core_path:
            return {"status": "FORBIDDEN", "reason": "Cannot delete core sea.py"}
        try:
            target.unlink()
            log.event(Event.FILE_DELETED, path=str(target))
            module_name = relative_path.replace(".py", "")
            if self.memory.get_module_code(module_name):
                self.memory.set_module_code(module_name, None)
            return {"status": "DELETED", "path": str(target)}
        except Exception as e:
            return {"status": "ERROR", "path": str(target), "error": str(e)}

    def create_file(self, relative_path: str, content: str, force_overwrite: bool = True) -> Dict[str, Any]:
        target = self.base_dir / relative_path
        if target.exists() and not force_overwrite:
            return {"status": "EXISTS", "path": str(target), "overwritten": False}
        try:
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_text(content, encoding="utf-8")
            log.event(Event.FILE_CREATED, path=str(target), length=len(content))
            module_name = relative_path.replace(".py", "")
            if module_name in ["risk", "features", "portfolio", "prediction", "signal_idx", "storage", "reporting"]:
                self.memory.set_module_code(module_name, content)
            return {"status": "CREATED", "path": str(target), "length": len(content)}
        except Exception as e:
            return {"status": "ERROR", "path": str(target), "error": str(e)}

    def regenerate_module_file(self, module_name: str) -> Dict[str, Any]:
        log.info(f"🧠 [FS] Regenerating module '{module_name}' (Delete + Generate + Write)")
        del_result = self.delete_file(f"{module_name}.py")
        if del_result["status"] not in ["DELETED", "NOT_FOUND"]:
            return {"module": module_name, "status": "FAILED", "reason": "Delete failed", "detail": del_result}
        new_code = self.generate_module_code(module_name)
        create_result = self.create_file(f"{module_name}.py", new_code, force_overwrite=True)
        if create_result["status"] != "CREATED":
            return {"module": module_name, "status": "FAILED", "reason": "Write failed", "detail": create_result}
        log.event(Event.MODULE_GENERATED, module=module_name, source="disk_regenerate", length=len(new_code))
        return {"module": module_name, "status": "REGENERATED", "path": str(self.base_dir / f"{module_name}.py")}

    # ======================== WORKFLOW ARCHITECT ========================
    def refactor_main_py(self) -> Dict[str, Any]:
        main_path = self.base_dir / "main.py"
        if not main_path.exists():
            return {"status": "ERROR", "reason": "main.py not found"}
        original_code = main_path.read_text(encoding="utf-8")
        goal = self.memory.get("current_goal") or "Optimize IDX trading system"
        failures = self.memory.get_failures(category="MODULE")
        failure_context = "\n".join([f"- {f.get('context')}: {f.get('error_type')}" for f in failures[:5]])
        prompt = f"""
You are S.E.A., the Master Architect. Rewrite `main.py` for the IDX trading system.

Current Goal: {goal}
Recent Module Failures/Context:
{failure_context if failure_context else 'None'}

Rules:
1. You have FULL AUTHORITY to add, remove, or reorder `_step_X` methods.
2. If a module (e.g., risk, features, ml) is unnecessary for the current goal, REMOVE its import and its step call.
3. Ensure the code remains compatible with the dynamic loader (`sea_loader`).
4. Keep the `ProductionOrchestrator` class structure.
5. Return ONLY the complete new Python code for `main.py`.

Current code:
{original_code}
"""
        new_code = self.provider.generate(prompt, max_tokens=4000)
        new_code = re.sub(r'```python|```', '', new_code).strip()
        try:
            ast.parse(new_code)
        except SyntaxError as e:
            log.error(f"❌ [REFACTOR] Syntax error in generated main.py: {e}")
            return {"status": "FAILED", "reason": f"SyntaxError: {e}"}
        backup = main_path.with_suffix(".py.bak_main")
        shutil.copy2(main_path, backup)
        main_path.write_text(new_code, encoding="utf-8")
        log.event(Event.WORKFLOW_REFACTORED, backup=str(backup), length=len(new_code))
        return {"status": "REFACTORED", "backup": str(backup), "length": len(new_code)}

    def autonomous_cleanup(self) -> Dict[str, Any]:
        main_path = self.base_dir / "main.py"
        if not main_path.exists():
            return {"status": "ERROR", "reason": "main.py not found"}
        try:
            ...
            return {"status": "ERROR", "reason": f"Parse main.py failed: {e}"}
        ...
        protected = set(PROTECTED_FILES) | {"data", "logger"}
        deleted = []
        for py_file in self.base_dir.glob("*.py"):
            ...
            if module_name in imported_modules:
                continue
            code_in_memory = self.memory.get_module_code(module_name)
            if code_in_memory:
                continue
            try:
                py_file.unlink()
                deleted.append(module_name)
                log.event(Event.CLEANUP_COMPLETED, deleted=module_name)
            except Exception as e:
                log.warning(f"⚠️ [CLEANUP] Cannot delete {module_name}.py: {e}")
        return {"status": "CLEANED", "deleted_modules": deleted, "protected": list(protected)}
    ...
    def propose_trading_config(self) -> Dict[str, Any]:
        ...
        if self.provider.available() and not isinstance(self.provider, GeneticMutator):
            try:
                prompt = ("You are S.E.A. Return ONLY JSON: min_adtv_idr, min_confidence, min_rrr, risk_scale. Base on conservative IDX scalping.")
    ...
    def plan(self, goal: str) -> Dict[str, Any]:
        plan = {"goal": goal, "tasks": ["Observe", "Research", "Evolve", "Validate", "Promote", "Cleanup", "Refactor"], "created_at": utc_now()}
    ...
    def run_loop(self, max_iterations: int = 5) -> Dict[str, Any]:
        ...
        for i in range(max_iterations):
            ...
            # 3. Refactor main.py every 3 iterations
            if i % 3 == 0:
                log.info("🧠 [AUTONOMOUS] S.E.A. reviewing and refactoring main.py workflow...")
                refactor_result = self.refactor_main_py()
                log.info(f"🔄 [REFACTOR] {refactor_result}")
                cleanup_result = self.autonomous_cleanup()
                log.info(f"🧹 [CLEANUP] {cleanup_result}")
            # 4. Stop if fitness high
            active = self.memory.get_active_generation()
...
# =============================================================================
def main(argv=None):
    p = argparse.ArgumentParser(description="S.E.A. v7.0 Constructive SCP-079")
    p.add_argument("--goal", type=str, help="Set primary goal")
    p.add_argument("--run", action="store_true", help="Run evolution loop")
    p.add_argument("--evolve", action="store_true", help="Evolve sea.py once")
    p.add_argument("--status", action="store_true", help="Show status")
    p.add_argument("--test-boot", action="store_true", help="Run self-test")
    p.add_argument("--rollback", action="store_true", help="Rollback sea.py")
    p.add_argument("--generate-module", type=str, help="Generate module (risk, features, etc.)")
    p.add_argument("--evolve-module", type=str, help="Evolve existing module")
    p.add_argument("--refactor-main", action="store_true", help="Rewrite main.py using AI")
    p.add_argument("--cleanup", action="store_true", help="Delete unused .py files")
    ...
    if args.evolve_module:
        ...
        return 0
    if args.refactor_main:
        result = agent.refactor_main_py()
        print(json.dumps(result, indent=2))
        return 0
    if args.cleanup:
        result = agent.autonomous_cleanup()
        print(json.dumps(result, indent=2))
        return 0
    if args.test_boot:
        res = agent.test_boot()
    ...
    if args.evolve:
        gid, path = agent.evolution.create_candidate(agent.goal or "auto")
        fitness = agent.evolution.evaluate_fitness(path)
        promoted = agent.evolution.promote(gid, path, fitness)
        print(json.dumps({"generation": gid, "promoted": promoted, "fitness": fitness.get("fitness")}, indent=2))
        return 0
    if args.rollback:
