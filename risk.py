"""
=============================================================================
IDX Stock Quantitative Trading System - Unified Risk Management Engine v2026.Q3.v3.2.1
Module Name : risk.py (Paper-Trading Adaptive & Polars-Native Audit Edition)
...
11. GeminiRiskDiagnosisEngine: AI Risk Diagnostic & Advisory Meta-Layer berbasis Google Gemini API.
12. UnifiedRiskEngine       : Facade Class terpusat sebagai single entry point pengawasan risiko (Polars & Dict support).
"""
...
IDX_DEFAULT_MIN_24H_VOLUME_IDR: float = 500_000.0 # Batas Minimal Turnover Live (Rp 500 Ribu)
IDX_DRYRUN_MIN_24H_VOLUME_IDR: float = 250_000.0  # Batas Minimal Turnover Dry-Run (Rp 250 Ribu)
...
class AnomalyDetector:
    ...
    def evaluate(self, df: pl.DataFrame, pipeline_timestamp: str, execution_id: str, trace_id: str = "") -> AnomalyResult:
        ...
        halt = anomaly_score >= 0.88
        warning = (anomaly_score >= 0.55) and not halt
...
class DrawdownGuard:
    ...
    def __init__(
        self,
        max_unrealized_dd: float = 0.25,
        max_realized_dd: float = 0.20,
    ...
    def evaluate(
        ...
        if unrealized_equity is None or unrealized_equity.len() == 0:
            unrealized_equity = pl.Series("unrealized_equity", [10_000_000.0])
...
# =============================================================================
# 4. LIQUIDITY GUARD (FLEXIBLE ADTV FOR SIMULATION)
# =============================================================================
@dataclass(frozen=True)
...
class LiquidityGuard:
    ...
    def __init__(
        ...
        min_value_traded_idr: float = IDX_DRYRUN_MIN_24H_VOLUME_IDR,
    ...
    def evaluate(self, df: pl.DataFrame, pipeline_timestamp: str, execution_id: str, trace_id: str = "") -> LiquidityResult:
        ...
        # Halt threshold dilonggarkan untuk simulasi
        halt = (final_liquidity_risk >= 0.92) or (median_value_traded < self.min_value_traded_idr * 0.01)
        warning = (final_liquidity_risk >= 0.65) and not halt
...
class OperationalStateMachine:
    ...
    def evaluate_metrics(self, risk_score: float, psi_score: float, timestamp: str, trace_id: str = "", run_id: str = "", session_id: str = "") -> SystemState:
        with self._lock:
            ...
            elif risk_score >= 0.60 or psi_score > 0.35:
...
class VolatilityGuard:
    ...
    def evaluate(self, df: pl.DataFrame, pipeline_timestamp: str, execution_id: str, trace_id: str = "") -> VolatilityResult:
        ...
        if len(rolling_vols) > 5:
            ...
            adaptive_thresh = max(composite_vol * 1.5, 0.50)
            
        adaptive_thresh = max(adaptive_thresh, 0.30)
...
class RiskAggregator:
    ...
    def aggregate(
        ...
        if liquidity_res.halt and not self.is_dry_run:
            hard_halt_sources.append("LIQUIDITY_COLLAPSE")
            highest_severity = "CRITICAL"
        elif liquidity_res.halt and self.is_dry_run:
            soft_halt_sources.append("SIMULATION_LOW_TURNOVER")
            
        if anomaly_res.halt and not self.is_dry_run:
            hard_halt_sources.append("MANIPULATION_ANOMALY")
            highest_severity = "HIGH"
        elif anomaly_res.halt and self.is_dry_run:
            soft_halt_sources.append("SIMULATION_ANOMALY_WARNING")
        ...
        # Toleransi skor komposit pada dry run dilonggarkan ke 0.90
...
class GeminiRiskDiagnosisEngine:
    """
    Sub-Engine Google Gemini AI sebagai Risk Diagnostic & Circuit Breaker Layer.
    Mengevaluasi metrik komposit risiko dan memberikan rekomendasi kualitatif/tindakan mitigasi.
    """
    ...
    def __init__(self, api_key: Optional[str] = None) -> None:
        ...
        if HAS_GEMINI_SDK and self.api_key:
            try:
                ...
                logger.info("🤖 [GEMINI_RISK_INIT] Gemini Client terhubung untuk Diagnostik Risiko.")
            except Exception as err:
                logger.warning(f"⚠️ [GEMINI_RISK_INIT_FAILED] Gagal inisialisasi Gemini Client: {err}")

    def run_risk_diagnosis(
        self,
        aggregated_risk: AggregatedRiskOutput,
        anomaly_res: AnomalyResult,
        volatility_res: VolatilityResult,
        liquidity_res: LiquidityResult,
        drawdown_res: DrawdownResult
    ) -> Dict[str, Any]:
        """Melakukan analisis diagnostik risiko kualitatif berbasis Google Gemini AI."""
        if not self.client:
            return {
                "ai_risk_diagnosis": "Gemini AI Client tidak aktif.",
                "ai_action_recommendation": "Lanjutkan eksekusi berdasarkan rule-based guardrail.",
                "urgency_level": "LOW"
            }

        prompt = f"""
        Sebagai Chief Risk Officer (CRO) sistem perdagangan kuantitatif pasar saham BEI (IDX), berikan analisis diagnostik dan rekomendasi tindakan berdasarkan evaluasi risiko berikut:

        === METRIK EVALUASI RISIKO AGREGAT ===
        - Composite Risk Score: {aggregated_risk.composite_risk_score:.4f}
        - Final Halt Status: {aggregated_risk.final_halt}
        - Final Warning Status: {aggregated_risk.final_warning}
        - Severity Level: {aggregated_risk.highest_severity_triggered}
        - Hard Halt Sources: {aggregated_risk.hard_halt_sources}
        - Soft Halt Sources: {aggregated_risk.soft_halt_sources}

        === SUB-SYSTEM METRICS ===
        - Anomaly Score: {anomaly_res.anomaly_score:.4f} ({anomaly_res.anomaly_type})
        - Volatility Score: {volatility_res.volatility_score:.4f} (Realized: {volatility_res.realized_volatility*100:.1f}%)
        - Liquidity Score: {liquidity_res.liquidity_score:.4f}
        - Current Drawdown (Unrealized): {drawdown_res.current_unrealized_dd*100:.2f}%

        Tugas:
        1. Berikan diagnosa kualitatif singkat (1-2 kalimat) mengenai potensi krisis atau kondisi abnormalitas pasar.
        2. Berikan instruksi tindakan operasional (misal: "Normal Execution", "Reduce Position Size", "Emergency Halt & Cooldown").
        ...
        try:
            ...
            return {
                "ai_risk_diagnosis": parsed.get("diagnosis", "Diagnostik risiko berhasil."),
                "ai_action_recommendation": parsed.get("action_recommendation", "Pertahankan eksekusi sesuai batas toleransi."),
                "urgency_level": parsed.get("urgency_level", "LOW")
            }
        except Exception as err:
            logger.warning(f"⚠️ [GEMINI_RISK_DIAGNOSIS_FAILED] Error saat memanggil Gemini API: {err}")
            return {
                "ai_risk_diagnosis": "Gagal memperoleh respon AI Gemini.",
                "ai_action_recommendation": "Ikuti protokol circuit breaker bawaan.",
                "urgency_level": "MEDIUM" if aggregated_risk.final_warning else ("CRITICAL" if aggregated_risk.final_halt else "LOW")
            }


# =============================================================================
# 12. UNIFIED RISK ENGINE (FACADE CLASS WITH DUAL POLARS & DICT SUPPORT)
# =============================================================================
class UnifiedRiskEngine:
    ...
    def __init__(
        ...
        # Deteksi otomatis mode simulasi
        run_mode = str(os.getenv("RUN_MODE", os.getenv("EXECUTION_MODE", ""))).lower()
        cli_args = " ".join(sys.argv).lower()
        self.is_dry_run = (
            "dry" in self.mode or "dry_run" in run_mode or "dryrun" in run_mode or 
        ...
        self.liquidity_guard = LiquidityGuard(min_value_traded_idr=active_min_adtv)
    ...
    def evaluate_market_risk(
        ...
        # Evaluasi Diagnostik risiko berbasis Google Gemini AI
        ai_diag = self.gemini_engine.run_risk_diagnosis(
            aggregated_output, anomaly_res, volatility_res, liquidity_res, drawdown_res
        )
        
        # Inject AI diagnosis ke telemetry hasil agregasi
        aggregated_output.telemetry["ai_diagnosis"] = ai_diag
    ...
    def audit_portfolio_orders(self, orders: Any, aggregated_risk: Optional[AggregatedRiskOutput] = None) -> Any:
        ...
        Mendukung input berformat Polars DataFrame, List of Dicts, maupun Dict Tunggal.
        """
        if orders is None:
            return None

        # Handle Circuit Breaker Global Hard Halt
        ...
        # ---------------------------------------------------------------------
        # CASE A: POLARS DATAFRAME SUPPORT (Standard Main.py Pipeline)
        # ---------------------------------------------------------------------
        if isinstance(orders, pl.DataFrame):
            if orders.is_empty():
                return orders
            ...
            cols = orders.columns
            price_col = next((c for c in ["entry_price", "price", "close", "current_price"] if c in cols), None)
            notional_col = next((c for c in ["notional_idr", "notional", "notional_usdt"] if c in cols), None)

            filtered_df = orders
            if price_col:
                filtered_df = filtered_df.filter(pl.col(price_col) >= IDX_MIN_PRICE_IDR)
            
            if notional_col:
                filtered_df = filtered_df.filter((pl.col(notional_col) == 0.0) | (pl.col(notional_col) >= IDX_MIN_NOTIONAL_IDR))

            if "selfTradePreventionMode" not in filtered_df.columns:
                filtered_df = filtered_df.with_columns(pl.lit(IDX_DEFAULT_STP_MODE).alias("selfTradePreventionMode"))

            return filtered_df

        # ---------------------------------------------------------------------
        # CASE B: LIST OF DICTS / SINGLE DICT SUPPORT
        # ---------------------------------------------------------------------
        ...
        for order in orders_list:
            if not isinstance(order, dict):
                continue
            price = float(order.get("entry_price", order.get("price", order.get("close", 0.0))))
            qty = float(order.get("quantity", order.get("qty", 0.0)))

            if price > 0.0 and price < IDX_MIN_PRICE_IDR:
                continue
            notional = price * qty if qty > 0 else float(order.get("notional_idr", order.get("notional", 0.0)))
        ...
        if isinstance(orders, dict) and validated_orders:
