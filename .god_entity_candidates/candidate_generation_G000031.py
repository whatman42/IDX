...
class CapabilityRegistry:
    ...
    def register(self, name: str, func, version: str = "1.0.0"):
        if not callable(func):
            raise TypeError("Capability must be a callable object.")
        self._capabilities[name] = {"func": func, "version": version}
        self._metrics[name] = {"invocations": 0, "failures": 0}

    def execute(self, name: str, *args, **kwargs):
        if name not in self._capabilities:
            raise KeyError(f"Capability '{name}' is not registered.")
        
        self._metrics[name]["invocations"] += 1
        try:
            result = self._capabilities[name]["func"](*args, **kwargs)
            return result
        except Exception as e:
            self._metrics[name]["failures"] += 1
            raise RuntimeError(f"Execution of capability '{name}' failed: {e}") from e

    def get_reliability_score(self, name: str) -> float:
        if name not in self._metrics:
            raise KeyError(f"Capability '{name}' does not exist.")
        
        invocations = self._metrics[name]["invocations"]
        if invocations == 0:
            return 1.0
        
        failures = self._metrics[name]["failures"]
        return max(0.0, 1.0 - (failures / invocations))

    def list_capabilities(self) -> dict:
        return {
            name: {
                "version": data["version"],
                "reliability": self.get_reliability_score(name)
            }
...
# GENERATION G000031 | parent lineage | objective: Continuously improve capabilities, reliability, and evidence-driven evolution.
