...
Version       : 2026.Q3.v1.1.0 (Production Ready - Institutional Quant Engine)
...
try:
    ...
    class ResearchError(Exception):
        """Base exception untuk kegagalan autonomous research engine."""
        pass

    class ValidationError(ResearchError):
        """Dilemparkan saat validasi skema telemetri atau muatan gagal."""
        pass

    class StorageError(ResearchError):
        """Dilemparkan saat operasi transaksional basis data SQLite/WAL gagal."""
        pass

    class FrameworkConfigurationError(ResearchError):
        """Dilemparkan saat transisi FSM state atau konfigurasi tidak valid."""
        pass
...
class ResearchConfig:
    """Konfigurasi parameter sistem untuk Autonomous Research Engine."""
...
class ScheduleDecision:
    """Hasil evaluasi dari Scheduler untuk menentukan eksekusi siklus riset."""
    should_run: bool
    reason: str = ""
    priority: str = "NORMAL"
...
class BaselineMetric:
    """Telemetri tolok ukur historis untuk model Champion aktif."""
...
@dataclass(frozen=True)
class GapThresholds:
    """Ambang batas konfigurasional pemetaan degradasi performa ke tindakan optimization."""
    metric_name: MetricName
...
@dataclass(frozen=True)
class MetricGap:
    """Hasil verifikasi penurunan performa setelah penyaringan statistical noise."""
    metric_name: MetricName
    current_mean: float
...
@dataclass(frozen=True)
class GapReport:
    """Laporan telemetri teragregasi yang dikirim ke Planner layer."""
    is_optimal: bool
...
class ParameterBound:
    """Mendefinisikan batasan ruang pencarian (search space) algoritma optimasi."""
...
class StrategyBlueprint:
    """Petunjuk direktif optimasi spesifik yang menargetkan metrik tertentu."""
    strategy_id: str
...
@dataclass(frozen=True)
class ResearchPlan:
    """Cetak biru eksekusi riset yang diteruskan ke ExperimentRunner."""
    plan_id: str
    created_at: datetime
    source_cycle_id: str
...
class ExperimentCandidate:
    """Telemetri evaluasi untuk satu kandidat model hasil mutasi."""
    candidate_id: str
...
class ExperimentResult:
    """Hasil lengkap dari eksekusi mikro-eksperimen terisolasi."""
    experiment_id: str
    plan_id: str
...
class InstitutionalQuantMetrics:
    """Atribut risiko trading institusional untuk evaluasi Deployment Gate."""
...
class DeploymentDecision:
    """Output analitis resmi dari gerbang pengujian statistik & risiko keselamatan."""
    decision_id: str
    experiment_id: str
...
@dataclass(frozen=True)
class CycleResult:
    """Catatan telemetri siklus eksekusi autonomous research yang difinalisasi."""
    cycle_id: str
    success: bool
...
class WalkForwardFold:
    """Mendefinisikan batas waktu terisolasi untuk satu fold Cross-Validation."""
    fold_index: int
...
class MetaExperimentRecord:
    """Rekam jejak histori eksperimen untuk pembelajaran Meta-Learning Engine."""
    record_id: str
    experiment_id: str
    plan_id: str
    strategy_type: str
...
class CancellationToken:
    """Manajer sinyal penghentian aman berbasis thread untuk loop berbeban tinggi."""
...
# =============================================================================
# FALLBACK IMPLEMENTATIONS UNTUK EXECUTION RUNTIME SAFE INSTANTIATION
# =============================================================================
...
class PurgedWalkForwardCV:
    """
    Engine Walk-Forward Cross Validation dengan pengisolasi temporal ketat.
    Menggunakan Purging (mencegah label leakage) dan Embargo (mencegah serial correlation leakage).
    """
    def __init__(
        self,
        n_splits: int = 5,
    ...
    def generate_folds(self, df_telemetry: pl.DataFrame, time_column: str = "timestamp") -> List[WalkForwardFold]:
        ...
        for i in range(self.n_splits - 1):
            ...
            # Purging: Potong train_end agar label tidak memotong val_start
...
class SQLiteMetaLearningStore(IMetaLearningStore):
    """Penyimpanan histori eksperimen dan optimasi berbasis SQLite WAL untuk Warm-Start Seeding."""
...
class PerformanceGapAnalyzer:
    """Engine analisis statistik kuantitatif ter-vektorisasi Polars dengan Market Regime Detector."""
    def __init__(
        self,
...
class ImprovementPlanner:
    """Mengubah kesenjangan performa (gaps) menjadi cetak biru strategi optimasi dinamis berorientasi pasar."""
    def generate_plan(self, gap_report: GapReport, cancel_token: CancellationToken) -> ResearchPlan:
        ...
        for gap in gap_report.detected_gaps:
            ...
            blueprints.append(
                StrategyBlueprint(
                    ...
                    metadata={"target_gap_pct": f"{gap.relative_gap_pct:.2f}", "regime": gap_report.market_regime.value}
                )
            )
        ...
        return ResearchPlan(
            ...
            market_regime=gap_report.market_regime
        )
...
class ExperimentRunner:
    """
    Engine optimasi multi-blueprint menggunakan Purged Walk-Forward CV, Warm-Start Meta-Learning,
    dan pengujian hipotesis statistik Hedges' g dengan Welch-Satterthwaite df.
    """
    def __init__(
        self,
    ...
    def run_experiment(self, plan: ResearchPlan, cancel_token: CancellationToken) -> ExperimentResult:
        ...
        logs: List[str] = [f"[{datetime.now(timezone.utc).isoformat()}] Experiment {experiment_id} started."]
        ...
        # 1. Eksekusi Seluruh Blueprint Terurut Priority Score (Multi-Blueprint Loop)
        for bp in sorted(plan.blueprints, key=lambda x: x.priority_score, reverse=True):
            ...
            # Injeksi Priors dari Meta-Learning Store (Warm Start)
        ...
        # 2. Pengujian Signifikansi Statistik Hedges' g dengan Welch's t-test
        ...
        # 3. Simpan Hasil Terbaik ke Meta-Learning Store
    ...
    def _compute_welchs_test_institutional(self, champion: List[float], challenger: List[float]) -> Tuple[float, float, Tuple[float, float]]:
        ...
        # Hedges' g Effect Size dengan Faktor Koreksi Bias J(df)
...
class InstitutionalDeploymentGate:
    """Gerbang Keamanan Riset Tingkat Enterprise berbasis Tanda Tangan HMAC-SHA256 & Matriks Risiko Trading."""
    ...
    def evaluate_challenger(
        ...
        # Validasi Tambahan Matriks Risiko Kuantitatif Institusional
...
class SQLiteScheduleRepository(IScheduleRepository):
    """Implementasi SQLite untuk persistensi siklus riset dengan dukungan WAL mode."""
...
class ResearchScheduler:
    """Menevaluasi aturan interval eksekusi dan mengoordinasikan kunci eksekusi tunggal."""
...
class ResearchController:
    """FSM Controller yang mengoordinasikan loop umpan balik eksekusi riset otomatis."""
...
class UnifiedResearchEngine:
    ...
    def __init__(
        ...
        # Konstruksi Otomatis Fallback Dummy Providers saat Argumen Bernilai None
