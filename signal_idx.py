"""
================================================================================
MODULE: signal_idx.py (v2026.5.16 Production Resilience & Strict Schema Fix)
...
IDX_DEFAULT_MIN_24H_VOLUME_IDR: Final[float] = 1_000_000_000.0 # Standard ADTV Threshold (Rp 1 Miliar)
IDX_DRYRUN_MIN_24H_VOLUME_IDR: Final[float] = 100_000_000.0    # Dry-Run ADTV Threshold (Rp 100 Juta)
...
class SignalGenerator(_SignalBaseEngine):
    def __init__(self, engine_id: str, operational_config: Dict[str, Any]) -> None:
        super().__init__(engine_id, "v2026.5.16")
    ...
    def generate(self, upstream_df: pl.DataFrame) -> pl.DataFrame:
        ...
        if "prediction_probability" not in cols and "probability" not in cols:
            logger.warning("[Gateway 1] Missing probability column. Auto-imputing with 0.50 fallback.")
            adapter_exprs.append(pl.lit(0.50).cast(pl.Float64).alias("prediction_probability"))
            adapter_exprs.append(pl.lit(0.50).cast(pl.Float64).alias("probability"))
        ...
        if "prediction_confidence" not in cols and "confidence" not in cols:
            logger.warning("[Gateway 1] Missing confidence column. Auto-imputing with probability/0.50 fallback.")
            prob_source = "prediction_probability" if "prediction_probability" in cols else "probability"
            if prob_source in cols:
                adapter_exprs.append(pl.col(prob_source).cast(pl.Float64).alias("prediction_confidence"))
                adapter_exprs.append(pl.col(prob_source).cast(pl.Float64).alias("confidence"))
            else:
        ...
        if adapter_exprs:
            ...
            cols = list(df_adapted.columns)  # Update daftar kolom
        ...
        # 6. ARA-Aware TP Calculation & Lower-Bound Tick Guarantee
        ...
        raw_tp = pl.col("current_price") + (atr_expr * 1.8)
...
class ExitFilter(_SignalBaseEngine):
    ...
    def filter_exit(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        min_rr = float(self._config.get("min_risk_reward_ratio", 1.0))
...
class TpSlOptimizer(_SignalBaseEngine):
    ...
    def optimize_tp_sl(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        total_equity_idr = float(self._config.get("portfolio_equity_idr", 100_000_000.0))
        max_risk_pct = float(self._config.get("max_risk_per_trade_pct", 0.01))
        max_pos_idr = float(self._config.get("max_position_idr", 10_000_000.0))
...
class SignalExplainer(_SignalBaseEngine):
    ...
    def explain(self, input_df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Sanitasi variabel masukan agar pl.format() tidak menghasilkan Null
        ...
        audit_json = pl.struct([
            ...
            pl.lit("v2026.5.16").alias("engine_version")
        ]).struct.json_encode()
...
class UnifiedSignalEngine:
    FACADE_VERSION: Final[str] = "v2026.5.16"
    ...
    def __init__(self, engine_id_prefix: str = "IDXSignalEngine", custom_configs: Optional[Dict[str, Dict[str, Any]]] = None) -> None:
        ...
        default_cfg = {
            "generator": {"min_24h_volume_idr": active_min_adtv},
            "entry_filter": {}, 
            "exit_filter": {"min_risk_reward_ratio": 1.0}, 
            "prob_filter": {"min_prediction_probability": 0.50}, 
            "conf_filter": {"min_prediction_confidence": 0.50},
            "tpsl_optimizer": {
                "portfolio_equity_idr": 100_000_000.0,
                "max_risk_per_trade_pct": 0.01,
                "max_position_idr": 10_000_000.0
            },
            "numerical_sanity": {},
            "ranker": {}, 
            "validator": {}, 
    ...
    def execute_pipeline(self, prediction_df: pl.DataFrame) -> pl.DataFrame:
        ...
        try:
            ...
            # Fallback Skema Utuh untuk Kompatibilitas Downstream (main.py, portfolio.py, reporting.py)
            fallback_exprs = [
