...
class CapabilityMetrics:
    ...
    def record_execution(self, capability: str, success: bool, latency: float, evidence: Optional[Dict[str, Any]] = None) -> None:
        self.execution_counts[capability] = self.execution_counts.get(capability, 0) + 1
        if success:
            self.success_counts[capability] = self.success_counts.get(capability, 0) + 1
        
        if capability not in self.latency_records:
            self.latency_records[capability] = []
        self.latency_records[capability].append(latency)
        ...
        if evidence:
            ...
            logger.debug(f"Recorded evidence for {capability}: success={success}, latency={latency:.4f}s")

    def get_reliability_score(self, capability: str) -> float:
        total = self.execution_counts.get(capability, 0)
        if total == 0:
            return 0.0
        successes = self.success_counts.get(capability, 0)
        return successes / total

    def get_average_latency(self, capability: str) -> float:
        latencies = self.latency_records.get(capability, [])
        if not latencies:
            return 0.0
    ...
    def summarize(self) -> Dict[str, Any]:
        ...
        for cap in self.execution_counts:
            summary[cap] = {
                ...
                "reliability": self.get_reliability_score(cap),
                "avg_latency": self.get_average_latency(cap)
            }
        return summary


class ReliableExecutionEngine:
    """Executes callables with automatic retry, telemetry, and evidence collection."""

    def __init__(self, metrics: Optional[CapabilityMetrics] = None) -> None:
        self.metrics = metrics or CapabilityMetrics()

    def execute(self, capability_name: str, func: callable, *args: Any, **kwargs: Any) -> Tuple[Any, bool, Optional[str]]:
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"Execution failed for capability '{capability_name}': {error_msg}")
            success = False
        finally:
            latency = time.time() - start_time
            evidence = {
                "args_hash": hashlib.sha256(json.dumps(str(args)).encode()).hexdigest(),
                "error": error_msg
            }
            self.metrics.record_execution(capability_name, success, latency, evidence)

        return result, success, error_msg
# ==== END ENHANCEMENT ====


# GENERATION G000027 | parent lineage | objective: Continuously improve capabilities, reliability, and evidence-driven evolution.
