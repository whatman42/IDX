...
FileName      : main.py
Version       : 2026.Q3.v26.1 (Synchronized Exceptions & Anti-Burn Gemini Engine)
...
# Import data engine & granular exceptions yang sesuai dengan data.py v27.1
from data import (
...
# ==============================================================================
# HARD SAFETY BOUNDS & CLAMPS FOR GEMINI AUTOCONFIG
# ==============================================================================
HARD_CONFIG_BOUNDS = {
...
def apply_hard_safety_clamps(raw_config: Dict[str, Any], logger_instance) -> Dict[str, Any]:
    """
    Memvalidasi dan menerapkan Hard Safety Clamps pada output Gemini.
...
if __name__ == "__main__":
    ...
    if args.bootstrap_universe:
        if not os.path.exists(DEFAULT_UNIVERSE_FILE):
            default_universe = ["BBCA.JK", "BBRI.JK", "BMRI.JK", "BBNI.JK", "TLKM.JK", "ASII.JK"]
            with open(DEFAULT_UNIVERSE_FILE, "w") as f:
                json.dump(default_universe, f, indent=2)
            print(f"✅ universe.json berhasil dibuat dengan {len(default_universe)} default ticker.")
        else:
            print("✔ universe.json sudah ada.")
        sys.exit(0)
